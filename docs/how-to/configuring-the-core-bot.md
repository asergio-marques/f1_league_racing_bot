...
> **And if a season is running, turning `results` off destroys that season's championship.** Every classification recorded, every standing computed from them, and every results and standings message already posted are deleted; every round still waiting on results, report verdicts or appeal verdicts is closed as having run without results, which is what lets you complete the season afterwards. Verdicts already announced stay in the verdicts channel — disabling takes back the results, not the decisions. Your points configurations and your division channels are kept. You are warned and must confirm, and none of it can be undone. See [Configuring the results module](configuring-the-results-module.md).
