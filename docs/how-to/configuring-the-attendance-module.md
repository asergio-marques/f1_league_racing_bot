...
**Check-in call** — the message the bot posts before a round asking a division whether its drivers are racing. It pings the division role, lists every driver team by team, and carries three buttons. There is one per round per division. A day after its round starts the bot deletes it, together with its reminder and its reserve-distribution message, and posting another round's call deletes nothing — so two rounds close together, a Saturday and Sunday double-header, can have both calls standing at once, each answerable until its own deadline. If the bot is switched off when that day runs out, it deletes them as soon as it starts again. The answers go nowhere with the messages: they stay in the attendance record, and the attendance sheet carries what they cost.
...
Either action is announced in the division's verdicts channel, the same place your penalty decisions go, so your league sees why a driver moved. The lineup post is redrawn to match, and the sheet for that same round is reposted straight away with the driver still listed and marked as having reached the limit — a sheet lists everyone who held a seat in the division this season, not only those who still do. An auto-sack also reposts the latest sheet of every **other** division the driver sat in, since they lose those seats too. These are headed like any other verdict — drawn as a banner where the image module's `Verdict banner` switch is on, written as text where it is off. If the sanction came out of approving a penalty review, it falls under that approval's header alongside the penalties; if it fired on its own — a clean round, or a pardon that made the bot re-check attendance — it gets one of its own.
...
> **An amendment rebuilds the round's attendance in both directions.** The corrected classification is what the round was, so adding somebody who was left out clears what they were charged, and taking somebody out who should never have been there marks them absent and charges them. If that takes a driver past the auto-reserve or auto-sack limit, the sanction is applied **now**, at the division's latest round — the bot cannot go back and apply it where the limit would first have been crossed, and it does not undo a sanction that already applied. Ordinary results submissions still err in the driver's favour: only an amendment or `/attendance sync` takes a recorded attendance back.
