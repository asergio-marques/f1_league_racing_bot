...
> **The posts do not quote your timings back.** Each one is titled by what it is — "Initial chance of rain", then "Initial session forecast", then "Final session forecast" — and says nothing about when it arrived or when the next is due. The first two promise a further forecast closer to the round, and the second tells drivers it will be the more accurate. So whatever you set here, the wording is right: move phase 1 to seven days and the post reads the same as it did at five.
...
> **A restart keeps them too.** When the bot starts again with forecasts still outstanding, it works out which ones it missed from your settings rather than from 5 days, 2 days and 2 hours. It used to use the standard timings, which meant a longer phase 1 was never published at all and a shorter one was published days early; it no longer does. It only catches up on rounds still to come, though: if you cancelled a round or a division, or the race itself went by while the bot was down, that round gets no forecast.
...
- **A cancellation note**, if a round, a division or the whole season is called off, saying no forecast is coming. It is posted silently: it sits in the channel but pings and pushes nobody. The notification drivers actually get is the check-in channel's, where attendance is on.

> **Nothing arrives with the module switched off.** The cancellation note, the mystery round notice and the invalidation notice are all this module's own output and wait for it. With weather off, a cancellation posts nothing in the forecast channel, and amending a round still throws its old forecasts away and deletes the messages that carried them, but nothing is posted in their place and no new forecast is drawn. Switch weather on afterwards and that round's forecasts are drawn then, as though they had never been attempted.
...
| A driver asks when the next forecast is due | The posts do not say, by design — they promise only that a closer and more accurate one follows. Your own deadlines are in `/season placements-review` |
