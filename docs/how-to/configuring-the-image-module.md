...
> **Your calendar needs a cancellation overlay on every round.** When you cancel a round, the bot reposts the calendar with that round covered, so a calendar drawing must say what "covered" looks like. Give every round an element with the id `round_<x>_cancelled` — usually a group holding a panel the size of the round's card and a word across it. Put it **last** in the round's group so it lies over everything else, and keep it inside the round's own space, above its crop point. The bot leaves it on a cancelled round and removes it from every other, so leave it visible in your file. The calendar that comes with the bot draws an almost opaque panel with **CANCELLED** across it; copy that if you want a starting point. It is **required**: a calendar drawing missing it for any round is refused when you name it and at every review, so a calendar of your own made before this change needs one added before the bot will accept it again.
>
> **Where you put it is checked too.** The bot never moves, resizes or restacks your overlay — it only keeps it or removes it — so it refuses a drawing whose overlay is drawn before the round's other fields (it would end up underneath them), sits outside `round_<x>_group`, falls at or below that round's crop point, or is too small or too far off to cover the round's fields. That last one is the mistake that catches people: copying round 1's overlay into every other round and forgetting to move it. The refusal names the round and what is wrong. An overlay drawn as freehand paths declares no box for the bot to measure, so only the first three are checked on one.
...
> **The standings drawings got wider.** They are now 1828 px across, from 1200 and 1128. The old columns could not hold the widest thing a cell can be asked to show — a `DSQ` with another outcome raised beside it — so it ran over into the next round, and nothing said so; each session column is 54 px wide now. The last 100 px of that is the team column, which a team's full name needed (#381) — the results, lineup and attendance drawings gained the same 100 px for the same reason. If you have re-laid a standings drawing of your own, give your columns the same room; nothing can check this for you.
...
**The standings preview shows you every case at once**, as far as your division's size allows, so you can judge each one before a season ever produces it. The points are invented along with the finishing order: both tables carry two entries level on points — second and third, split as a countback splits them — and one on no points. A round before them is invented as well, so one entry has gained places, one has lost them and the rest hold theirs, and every movement marker gets drawn. If your division has a reserve seated, they appear in the drivers' table as a driver that earlier round did not hold, and for one round they stand in for a driver who sat it out, while another team runs a car short and scores nothing. A large enough field also carries a disqualification beside the retirement and the non-starter, in the results preview as well as on the standings grid.
...
**Mid-season, the review draws the lineups the confirmation will post.** When you confirm the drivers of a window opened during the season, each division one of them joins gets its lineup posted again. The review draws that lineup with the new drivers already in it, although they appear in no other lineup until you confirm. A lineup that will not draw — a template with too few seats for the new team size, say — takes the confirm button away just as at the start of the season, and so does anything wrong with the module's configuration: a missing Inkscape, a broken drawing behind a switched-on output, an unset colour slot or the portrait settings.
