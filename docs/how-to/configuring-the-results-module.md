...
>
> **Anything the bot cannot remove, it tells you about.** A message it fails to delete — usually because it has lost a permission in that channel — is not counted as removed: the reply and the log channel list each one with a link, and you delete it by hand. If the erase stops part-way altogether, the waiting rounds are still closed, so you can still complete the season; the reply tells you the erase did not finish, and some of the season's messages may still be posted for you to remove.
>
