...
- A posting that fails shall be reported and shall not refuse the confirmation.
...
- Cancelling a season shall be refused while any of its divisions has an amendment of a round's results open, and the refusal shall name the round being amended and its channel. The drivers' history would otherwise be written from that amendment's corrections before they are approved, and is never rewritten. Decided 2026-09-21.
...
- A cancellation shall never discard a result. Only a round not yet run, or run but with its results not yet entered, may be cancelled — by itself, or by the cancelling of the division or season above it. A round further along shall keep its place and its results.
    - A round of the cancelled season whose results are entered but whose report or appeal verdicts are still open shall be made final, before the driver pass runs. Nothing else would move it — the verdict commands are refused once the season is cancelled — and a driver whose only round is one of these would otherwise have raced no final round, be deleted by the driver pass, and take the results the rule above keeps with them. Decided 2026-09-21.
...
- A division's calendar may be reposted on demand. A calendar already posted shall not update itself, save upon a cancellation, as set out under Announcing a cancellation below. A round amended is carried onto it only when it is reposted.
...
- A round of a season in one of the three ongoing states may be cancelled upon the word `CONFIRM`. Its scheduled work shall be cancelled and the cancellation announced as set out below.
...
- Core shall post no notice of a cancellation of its own. Telling the drivers that a round, a division or a season is off is the league's to do; what the bot owes them is that no module goes on as though it were not, and that the calendar stops showing it as though it would be run. Decided 2026-09-19 (#175).
- Each enabled module shall say what the cancellation means for it, in its own channel and in its own terms, as its own specification sets out. A disabled module shall say nothing.
...
- Enabling test mode shall create and attach two ordinary points configurations, "Standard" and "Half Points", to the season in Configuration, unless a configuration of that name is already attached to it. They shall be created as ordinary configurations of the server and shall be indistinguishable from ones a league made itself, so that a test season passes the points requirement of confirming placements without one being built by hand.
    - Test mode shall attach them at no other moment. A season in test mode with no points configuration attached shall be named as a fault by both reviews and refused by the confirmation of the configuration and of placements, as any other season is. Decided 2026-09-23.
- Test mode shall not relax any requirement of confirming placements, the points configurations among them. In particular, a season holding a round whose moment has passed, or a round already inside one of its enabled modules' configured windows, shall be refused under test mode exactly as it is refused otherwise, so a test season built in the past shall not be confirmable — and the first of those shall refuse it with every module switched off. A test season that quietly lost its check-ins would misreport attendance precisely as a real one does, and is a worse thing to be testing against than a calendar that has to be moved forward.
