...
- Conduct investigation cycle - The full process for a Code of Conduct investigation. It may be initiated at any time, by the head steward or temporary head steward, or by any steward where the league allows it, against one or more users, with a complaint and evidence.
  - Defence submission - Active from the moment the investigation is opened, and disabled once a configured period of time elapses. In it, the involved users may submit defences and further evidence relevant to the case.
  - Investigation deliberation - Active once defence submission ends, and disabled once a configured period of time elapses. Aims to allow stewards to vote on the final verdict, providing justification. After this period is over, the verdict is posted as set out under Conduct cycle: the investigation is private, and its verdict public.
...
- Involved driver - A driver who is party to a ticket: for a report, its complainant where a driver, and every driver named in it or added to it since; for an appeal, the involved drivers of the report it appeals, and the appellant. For a CoC investigation, its involved users stand in their place.
...
- Appeal token - A special kind of currency that may be required for drivers to be able to submit an appeal. Appeal tokens are accumulated on a driver licence, and expire upon the current season's end. Upon a successful appeal, drivers are returned their spent tokens, unless the league has chosen otherwise.
- Code of Conduct investigation - May also be referred to as a CoC investigation. A special kind of ticket and the only one which is not linked to a round, but to one or more users instead. As it pertains to no session, its conduct outcomes hold no time penalty, disqualification, warning point or penalty point; a result may change only through an auto-rule its verdict triggers, as set out under Automated penalty rules. It may be initiated by the head steward or temporary head steward, and by any steward where the league allows it. It cannot be appealed, and any decisions made are final. This functionality is optional and is disabled by default.
...
- Time penalty - A possible direct outcome of a verdict for a report or appeal. Time is added to the driver's total race time. An appeal changing the initial verdict may take it away again, but never so far that the driver's in-race and post-race time penalties together fall below zero seconds.
...
- Championship points deduction - A possible direct outcome of a verdict for any ticket. A number of points is taken from the driver's total in the division's drivers' championship, their results left untouched; the points their team scored in the constructors' championship are kept.
- Championship disqualification - A possible direct outcome of a verdict for any ticket. The driver is removed from the division's drivers' championship for the season, their results left untouched, and they may race on unless also banned; the points their team scored in the constructors' championship are kept.
- Constructors' points deduction - A possible direct outcome of a verdict for any ticket, given to a team. A number of points is taken from the team's total in the division's constructors' championship; the points its drivers scored in the drivers' championship are kept.
- Constructors' championship disqualification - A possible direct outcome of a verdict for any ticket, given to a team. The team is removed from the division's constructors' championship for the season; the points its drivers scored in the drivers' championship are kept.
...
- <NEW COMMAND> A "steward team conflict-toggle" command will be made available to league managers, which shall determine whether stewards who are also drivers are able to participate in the stewarding cycle of tickets that pertain to divisions they are driving in, accepting a possible conflict of interest.
  ...
  - A CoC investigation pertains to no division; for it, a steward is held to drive in its divisions where they drive in any division in which an involved user races.
...
- <NEW COMMAND> A "steward outcome add" command will be made available to league managers, which shall open a form collecting the following:
  ...
  - Championship points deducted - Optional - Integer input only. Number of points taken from the driver's total in the drivers' championship.
  - Championship disqualification - Optional - Checkbox that, if ticked, means that the driver is removed from the drivers' championship for the season.
  - Constructors' points deducted - Optional - Integer input only. Number of points taken from a team's total in the constructors' championship.
  - Constructors' championship disqualification - Optional - Checkbox that, if ticked, means that a team is removed from the constructors' championship for the season.
  - Every penalty above is given to a driver, save the constructors' points deduction and the constructors' championship disqualification, which are given to a team, as set out under Tickets.
  ...
  - Season ban - Optional - Checkbox that, if ticked, means that the offending driver's licence will accrue a season ban, lasting as long as the league has chosen: a season's worth of rounds, until a season's completion, or a set number of days.
  ...
  - At least one of the "Time penalty", "Disqualification", "Championship points deducted", "Championship disqualification", "Constructors' points deducted", "Constructors' championship disqualification", "Warning point", "Penalty point", "Qualifying ban", "Race ban", "Season ban", "League ban" fields must be different from 0.
...
- By default, a permanent, unremovable, unmodifiable outcome is added to the list, with the following data. It is the one exception to the rule that an outcome sets at least one penalty other than 0, which governs the outcomes a league defines:
  ...
  - Time penalty - 0
  - Disqualification - No
  - Championship points deducted - 0
  - Championship disqualification - No
  - Constructors' points deducted - 0
  - Constructors' championship disqualification - No
  - Warning point - 0
  - Penalty point - 0
  - Qualifying ban - 0
  - Race ban - 0
...
- <NEW COMMAND> A "steward penalty penalty-point-expiry" command will be made available to league managers, which shall have as single input one of the behaviours of "steward penalty warning-point-expiry", determining alike when penalty points expire.
  - By default, this will be set to "number of rounds", the number being the length of the season.
- <NEW COMMAND> A "steward role league-ban" command will be made available to league managers, which shall have as mandatory single input a role to be attributed when any driver is league banned, and removed when any driver's league ban is revoked.
...
- As configured, a single-round rule will be interpreted as meaning "if a driver receives a certain amount of pre-defined penalty type in a single round, they will be handed out a number of penalties of a certain kind, the same as the one counted or another".
- <NEW COMMAND> A "steward auto-rule add-multi-round" command will be made available to league managers, which shall have no inputs. The final output of this command is a rule that, without any further user input, checks for configured criteria on the outcomes of a previous number of rounds, the latest round completed included, upon the closing of the stewarding cycle of any given round. Upon usage of this command, a form will open, collecting the following:
  - ID - Mandatory - String - Unique ID for this rule. Must not overlap with that of other auto rules, regardless of type. Maximum of 12 characters.
  - Infringement - Optional - String - An optional string for league managers to add a rule number to be printed out in the verdict.
  - Number of rounds - Mandatory - Integer - Number of previous rounds' outcomes that will be checked for the type of infractions committed by a same driver.
...
- As configured, a multi-round rule will be interpreted as meaning "if a driver receives a certain amount of pre-defined penalty type across the last X rounds, they will be handed out a number of penalties of a certain kind, the same as the one counted or another".
...
- As configured, an accumulation of active penalties rule will be interpreted as meaning "if a driver licence holds a certain amount of active penalties, meaning penalties not served or expired, of a pre-defined type, they will be handed out a number of penalties of a certain kind, the same as the one counted or another".
...
- The types an auto-rule may count, as its "Type of infractions committed", are warning points, penalty points, discipline points, time penalties, disqualifications, championship points deductions, championship disqualifications, qualifying bans, race bans and season bans. Time penalties are counted as the cumulative sum of their seconds, and championship points deductions as the sum of the points deducted; every other type by its number.
- The types an auto-rule may hand out, as its "Penalty given", are warning points, penalty points, discipline points, time penalties, disqualifications, championship points deductions, a championship disqualification, qualifying bans, race bans, a season ban and a league ban.
- An auto-rule judges one driver's licence, and neither counts nor hands out a penalty given to a team. A team is sanctioned only upon a ticket to which all its full-time drivers are party.
...
- <NEW COMMAND> A "steward conduct-outcome add" command will be made available to league managers, which shall open a form collecting the following:
  ...
  - Season ban - Optional - Checkbox that, if ticked, means that the offending driver's licence will accrue a season ban, lasting as long as the league has chosen: a season's worth of rounds, until a season's completion, or a set number of days.
  - League ban - Optional - Checkbox that, if ticked, means that the offending driver licence will accrue a league ban.
  - Championship points deducted - Optional - Integer input only. Number of points taken from the user's total in the drivers' championship of each division they race in.
  - Championship disqualification - Optional - Checkbox that, if ticked, means that the user is removed from the drivers' championship of each division they race in, for the season.
  - Constructors' points deducted - Optional - Integer input only. Number of points taken from a team's total in the constructors' championship.
  - Constructors' championship disqualification - Optional - Checkbox that, if ticked, means that a team is removed from the constructors' championship for the season.
  - Every penalty above is given to a user, save the constructors' points deduction and the constructors' championship disqualification, which are given to a team, as set out under Tickets.
  ...
  - This command shall fail if the outcome has any disabled penalty types.
  - At least one of the "Discipline points", "Championship points deducted", "Championship disqualification", "Constructors' points deducted", "Constructors' championship disqualification", "Qualifying bans", "Race bans", "Season ban", "League ban" fields must be different from 0.
...
- <NEW COMMAND> A "steward conduct-outcome list" command will be made available to league managers and stewards, which shall have no inputs. In reply, the bot will post a transient (temporary, seen only to the command user) list with all the conduct outcomes currently available, as a bullet point list as follows:
  ...
  - <brief>
    ...
    - Rule description: <description, if not empty, otherwise this line is skipped>
    - Associated outcome: <all penalties associated with the outcome, comma concatenated>
- By default, a permanent, unremovable, unmodifiable conduct outcome is added to the list, with the following data. It is the one exception to the rule that a conduct outcome sets at least one penalty other than 0, which governs the conduct outcomes a league defines:
  - ID - NFA (special reserved ID)
  - Brief - No Further Action
...
- Once this phase is entered, the following buttons will be posted on the channel after the header message is posted:
  ...
  - Remove driver - Can be used by members of the effective stewarding team for this ticket, by the one who initiated the ticket, and by its involved drivers. Takes a driver out of the list of involved drivers, and removes their permissions as an involved driver. When pressed, a form is opened so that one involved driver is named (mandatory) and a justification is given. The exact behaviour depends on who pressed it:
    ...
    - The command is rejected if the targeted user is the one who initiated the ticket, or if they are not in the involved drivers list, or if the ticket is already in report deliberation, appeal deliberation or investigation deliberation.
...
- When a driver is removed from a ticket, they will no longer be considered an involved driver, and the permissions of an involved driver will be removed from them.
...
### Deliberation
- A team may be sanctioned upon a ticket only where it holds at least one full-time driver in the division and every one of them is an involved driver of it, so that it is represented, and defended, by all of them. Reserve drivers are not counted. The Reserve team shall never be sanctioned, being a pool of reserve drivers and not a team of the league. The stewards may bring a missing team-mate into the ticket with "Add driver" before deliberation begins. In a CoC investigation, which pertains to no division, a team of any division of an ongoing season may be sanctioned where every full-time driver of it in its division is among the involved users.
...
- Once this phase is entered, a single button titled "Vote" is posted by the bot. This button will serve for members of the effective stewarding team to cast, modify, or remove their ballot on the outcome of the ticket. A ballot is one steward's whole view of the incident, or of the case. The button opens the steward's ballot, seen by them alone, which is as follows, save where a cycle sets out otherwise:
  ...
  - For each team that may be sanctioned upon the ticket:
    - Team outcome - Dropdown - Dropdown containing the outcomes the cycle offers that carry a team penalty, and "None", allowing the steward to select 1 of them. "None" by default.
...
- A ballot shall be counted whole. Two ballots are the same option only where they give every involved driver the same outcome and every team line the same team outcome, and no ballot shall be divided by driver or team for counting.
...
- If any one option reaches plurality without a tie, the ultimate result of the ticket will be that option: the outcome it gives each involved driver, and the team outcome it gives each team line.
...
- Once the report deliberation phase is considered over, the round results and the standings after the round will be reposted with what the report verdicts changed in them: their time penalties and disqualifications in the results, and their championship penalties in the standings. Points and bans go to the licences at the cycle close.

### Appeal submission
- A report may be appealed only while its round's appeal submission is open. Once it closes, the report's verdict is final, whether appealed or not.
...
- The rules for a ticket open to its parties, set out under Tickets, hold throughout this phase.
- If no appeal is lodged by the end of appeal submission, the round's stewarding cycle closes then, going through its cycle close as any other.
...
- The ballot of an appeal is as set out under Tickets, save that:
  ...
  - Its line for each team that may be sanctioned, the appeal's involved drivers being counted, is greyed out alike, and prefilled with the team outcome the initial verdict gave the team, or "None" where it gave none.
...
- A ballot with the decision "Change initial verdict" is only valid where it gives at least one involved driver an outcome, or one team a team outcome, other than the initial verdict gave them. A team may thus appeal its own penalty alone, and an appeal may give a team a penalty the report did not.
...
- Once the appeal deliberation phase is considered over, the round results and the standings after the round will be reposted with what the appeal verdicts changed in them, alike.
...
- Whether a qualifying ban or a race ban was served in a round shall be judged from the round's results, once its initial results are posted, and posted then as an Automated Ruling to the verdicts channel of the division, in a batch of its own. The driver licence shall be updated at once, so that a ban served no longer stands against the driver's next round, whenever that falls.
  ...
  - It is the one change a round makes to a licence before its stewarding cycle closes, the verdicts of the round's tickets reaching the licence only at the close. It only ever confirms that a ban carries on or clears it, and never imposes one.
...
### Qualifying bans
- Whether a driver has a qualifying ban is only determined at the close of a stewarding cycle or a conduct investigation cycle, once the auto-rules have been applied.
...
### Race bans
- Whether a driver has a race ban is only determined at the close of a stewarding cycle or a conduct investigation cycle, once the auto-rules have been applied.
...
- If the attendance module is enabled and a driver has a race ban to be served at a round, any answer they have given to that round's check-in call shall be discarded, and a notice posted to the division's check-in channel saying they cannot check in, being race banned.
  - The driver shall be unable to answer that check-in call again.
  - They shall not be given attendance points for not answering it: the bot shall give them an automatic attendance pardon, written to the log channel as any pardon is.
  - The notice shall be deleted alongside the check-in call it concerns, and posted again where that call is posted again.
...
- If a driver who has a race ban fails to serve it properly by being listed in the results of any session pertaining to the round in which they must serve the ban, then their race ban will be considered unserved, and will carry on to their next round in which it is to be served, as set out above.
...
### Season bans
- Whether a driver has a season ban is only determined at the close of a stewarding cycle or a conduct investigation cycle, once the auto-rules have been applied.
- A driver who gets a season ban shall be returned to Not Signed Up at once, as a sacking returns them: every seat they hold freed, full-time and reserve alike, across all divisions, and every division, team and signup role revoked.
...
- Whether a driver has a league ban is only determined at the close of a stewarding cycle or a conduct investigation cycle, once the auto-rules have been applied.
- A driver who gets a league ban shall be returned to Not Signed Up at once, as a sacking returns them: every seat they hold freed, full-time and reserve alike, across all divisions, and every division, team and signup role revoked.
...
- This section specifies how every verdict this module issues must be shaped and formatted: Report Verdicts, Appeal Verdicts, Code of Conduct Verdicts and Automated Rulings, whether an auto-rule triggered or a ban served or not served.
...
- A verdict shall notify only the involved drivers its decision names. A role mention, "@everyone" or "@here" written into any free text a ticket carries — a complaint, a justification, an incident or the grounds of an appeal — shall be refused when it is entered, telling the writer why. A mention of a driver stands, naming the other car being the ordinary thing to write.
...
- The image output will be similar to that of the attendance sheet, and consistent with the visual outputs the image module already draws. As every one of them does, it shall admit the division logo and the per-tier colours the image module provides, and a league's own logo is artwork of its template rather than a field. The following data elements shall be allowed:
  ...
  - Table-list of drivers - Mandatory, with the following columns:
    ...
    - Team name - Optional
    - Team logo - Optional
    - Active penalty points - Mandatory if penalty points are enabled
    - Active warning points - Mandatory if warning points are enabled
    - Active penalty points carried from earlier seasons - Mandatory if penalty points are enabled
    - Active warning points carried from earlier seasons - Mandatory if warning points are enabled
    - Column for each round - Optional, displays data as 1PP (penalty point), 1W (warning), QB (qualifying ban), RB (race ban), SB (season ban), LB (league ban), can show multiples of those
      - A cell holding a penalty may be drawn with a highlighted background.
