...
- Every command of this family shall be rejected where the server holds no season and the team list of the server holds no team beyond the reserve team, with a clear error naming the teams that are wanting and distinguishing itself from the rejection given above for a division holding none. This rejection applies to every command of the family, including those that draw no team and no driver, a fabricated league being built whole or not at all.
- The reply of a command drawing a fabricated league shall state that the server holds no season and that the league drawn is fabricated, and shall distinguish what was taken from the configuration of the server from what was invented.
