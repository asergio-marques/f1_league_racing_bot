...
- The default filename "lineup_template.svg" names a file that serves any league, as every other template of the module does. It is authored against a count of teams and of seats and never against a team list, so a file shipped with the bot draws a league whose teams it does not know. It declares eleven team blocks of two seat slots each, beside its reserve block of six slots, and declares "team_<x>_group" for each of those blocks.
...
- No field of this graphic depends upon the team list of a league, so every field of it is verifiable against the template alone, at every moment the template is verified. There is no moment at which the template can be checked only against a stand-in, and no divergence of this graphic is ever reported as a warning.
    - When the template is configured, it shall be verified that the template declares "division_name", that it declares at least one team block and at least one seat slot within it, each numbered continuously from 1, that the blocks declare "team_<x>_name" and "team_<x>_driver_<y>_name" throughout, and that it declares "reserve_group" with at least one reserve slot, numbered continuously from 1, of which the first declares "reserve_driver_1_name". A mandatory field that is absent is a fatal error rejecting the command.
