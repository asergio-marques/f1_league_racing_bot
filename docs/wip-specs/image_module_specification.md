...
- <MODIFY COMMAND> The "season review" command shall be augumented to display the enabling status of the images module, as well as all of the configurations above and if they are valid.
    ...
    - Every aspect not reported as enabled and valid shall state why beneath itself, and with each reason the command that addresses it. A warning sign states what is at fault, upon the terms above. A cross states that the aspect is switched off, that the posting it governs is made as text, and the command that switches it on; and shall further state what would need repairing were it switched on: the templates behind it that are at fault, each named individually; the module whose data it draws, where that module is itself switched off; and the absence of the rasteriser. What a cross promises shall be the same evaluation a warning sign would report were the aspect enabled, so that switching an aspect on can surprise no one.
    - A template at fault shall withhold the approval of a season only where the aspect drawing it is switched on. An aspect that is switched off posts as text and draws no template, so a template belonging only to such aspects shall be reported as a warning and shall withhold nothing. The season review and the season approval shall apply this one rule alike, so that what the review shows as blocking is exactly what the approval refuses upon.

- <MODIFY COMMAND> The "images config toggle" command shall verify, before switching an aspect on, every template that aspect draws, against the configured template directory. Where any is at fault the command shall be refused, naming each fault, and the aspect shall be left switched off. Switching an aspect off shall never be refused: the posting reverts to text, which draws nothing, and a league must always be able to retreat to it.
...
- <NEW COMMAND> A family of "images test" commands will be made available to league managers, one for each type of generation: calendar, lineup, results, standings, attendance, rsvp, weather-p1, weather-p2, weather-p3, weather-mystery, verdict.
    - The "images test calendar" and "images test lineup" commands take one parameter, the name of a division. Every other command of the family takes two, the name of a division and the number of a round. Both parameters are optional, being wanted only where the server holds a season to resolve them against; the rules of the family below govern when one is required and when it is disregarded.
...
- A member of a collection is distinguished by an ordinal, and by nothing else. No collection is distinguished by a datum of the league: a template shall be authorable in ignorance of the teams, the drivers and the tracks of any one league, and one file shall serve every league alike. A collection may also hold a single member bearing no distinguisher at all, as the reserve team of a lineup does; the name of such a member is reserved, and no field of a sibling collection may bear it.
- The members of a collection are numbered continuously from 1. A gap in the numbering is a fatal error.
- The capacity of a collection is fixed by the template: the number of members the template declares is the capacity, and the data are measured against it. The rounds of a calendar, the rows of a classification, the teams of a lineup, the seats of a team and the seats of a reserve team are all of this kind. A collection standing inside a member of another may instead be bounded by a configured value of that containing member, as stated below.
...
- A member the data hold but leave empty, such as a team that has recruited nobody, is not an unused member and is drawn empty rather than removed.
...
- The non-fatal errors of one generation shall be reported in one message and not in one message apiece, grouped by their kind, and errors identical to one another counted rather than repeated. A generation degrades in the same manner as many times as it draws the field that degrades, and a line for each buries the one error that differs among those that do not.
    - An error standing alone shall name the field it was met upon. One repeated shall name how many times it was met, in place of the fields, a list of them being nothing a league manager acts upon.
    - The message shall name what was being drawn — the championship, the session or the round — the type of the template alone not distinguishing one posting from another in a log holding many.
    - The record kept of the errors is unaffected, and holds one entry for each. The grouping governs what is read and nothing else.
- Where a command triggered the generation, the output of that command shall carry a link to the message written in the logging channel, so that the manager may follow it. The message shall therefore be written before the output of the command is composed. A logging channel that cannot be written to shall cost the link and nothing else; the output of the command shall be produced regardless.
- A report surveying the configuration — the season review and the "images config view" command — states every fault in the language of the league manager reading it. It shall carry no field identifier, no layer of verification, no file path and no name of a program, and shall say what is wrong in terms of the drawing and the folder the manager configured. The precise fault shall be written to the log of the bot, which is the only place it is kept. The two reports shall state a fault in the same words, as they state it upon the same evaluation.
    - An "images template X" command is not such a report and does not fall under this rule. It answers a manager naming one file, in the moment they can repair it, and states the field or the path at fault.
- Every fault such a report states shall be followed by the remedy for that fault: the command that addresses it, named in full and with the parameter that selects the thing at fault. A fault of a template names the "images template X" command of that template and not of its group; a fault of an asset folder names the "images config X-directory" command of that folder; an aspect that is switched off names the "images config toggle" command and the choice that selects it; a module switched off at its source names the "module enable" command and that module. Where a fault is answered by no command a league manager may run, as the absence of the rasteriser is, the report shall say so plainly rather than name a command they cannot use.
    - The name by which an aspect is offered in the "images config toggle" command and the label by which a report names that aspect shall be one and the same, so that a manager may use what they read.
- A report shall be divided across as many messages as its length requires. A report too long for one message is lost entirely and not merely truncated.
- A graphic the season review draws is drawn upon the data of the season being reviewed, so a fault met there is a fault every posting of that season will meet. Where the review meets one:
    - the fault shall be reported to the manager who invoked the review, and that section of the review posted in its traditional textual form, so that the review remains a complete account of the season;
    - the season shall not be offered for approval. The button the review ends with shall be withheld and replaced by a statement that the image module is not correctly configured, naming the review itself as what to run again once it is repaired.
- The approval of a season shall be refused upon the same evaluation, drawing every graphic the season would post and refusing while any of them will not draw, so that the review and the approval cannot disagree and a season cannot be committed past a fault by skipping the review. It shall name every division at fault and the graphic of it that failed, and shall refuse before anything of the season is committed.
    - Where the evaluation cannot be made at all, the approval shall be refused likewise. A season committed upon a check that never ran is the worse outcome of the two.
...
- A field emptied or removed because the league switched the collection of that datum off at its source is no error at all, and nothing shall be reported for it. Nothing has degraded: the graphic draws what the league configured it to draw. The suppression requires a configuration switch that turns the datum off at its source, as "signup nationality toggle" does for the nationality of a driver, and shall be stated for the field it applies to. It does not extend to a datum the league collects and merely happens not to hold for one member, which is an ordinary emptied field and reports as one.
    - While the test mode is active, the switch governing the nationality of a driver shall be the "test-mode nationality" switch and not the "signup nationality" switch, every place this module reads it. A manager may thereby see the graphics of a server under test both with flags and without them, the setting the signups of the league run upon being left as it stands.
    - A switch turning the collection of a datum off suppresses the datum itself and not merely the report of its absence: where it is off, no flag shall be drawn for any driver, one holding a nationality recorded before it was turned off included. The switch shall be read before the datum, at every place either is read, so that a preview and a posting of the same division draw the same thing.
- The moments at which a template is verified differ in the data available to them, and the severity of a divergence follows from that:
    ...
    - where the moment can compare the template only against a stand-in for the data that will be drawn, a divergence is a warning and the command succeeds. Season review compares a calendar template against the most demanding division of the season, the division actually drawn being decided later.
- No template of this module is authored against a datum of the league, every field of every graphic being addressed by an ordinal. A stand-in therefore stands in for how *many* members will be drawn, and never for *which*: a calendar template is compared against a round count and a lineup template against a count of teams and of seats, neither against a list of names.
...
- A graphic shall be named for what it depicts and never for the template that drew it, that name being the name of the file on disk and the name of the attachment a league receives alike. It is composed of the parts below, in this order, joined by underscores, each part omitted where it is not known:
    - "season<N>", N being the number of the season drawn;
    - "division<T>", T being the tier of the division drawn, or the name of that division reduced by the rule that names an asset file where the drawing carries no tier;
    - "round<Z>", Z being the number of the round drawn, omitted by the lineup and the calendar, which stand for a season and not for a round of one;
    - the subject of the graphic — "standings_drivers", "attendance", "weather_p1", "calendar", "checkin", "verdict" and the like. A classification names the session it draws rather than the template that drew it, one template serving four sessions.
- A season number, a tier or a round number of nought is a value not yet set and shall be omitted, not drawn as nought.
- The preview commands name their output by the same rule, drawing as they do for a division and a round of the league.
...
### Fonts
- A template declares its font as a CSS "font-family", on the text itself or on anything containing it — a group, or the drawing as a whole. A field declaring none shall take the declaration of the nearest thing above it that carries one, and a field's own declaration shall override an inherited one. The bot offers no font setting: a league changing the face its graphics are drawn in edits the template.
- A declaration may name several families in order of preference, and the first the machine carries shall be the one used. That list is the whole of the fallback mechanism, and the family measured against shall be the family the converter draws with. A generic name closing the list — "sans-serif" and its fellows — resolves to whatever the machine offers for it and is the last resort; what that is differs from one operating system to the next.
...
- A text field declares a **box**, which is the room the template gives it. Every field that may receive a value of a length the league does not control shall declare one: the name of a person, of a circuit, of a country, of a grand prix, and a date or a time. A field left unbounded does not overflow tidily — it runs across whatever is drawn beside it, and the graphic is wrong in a way no error reports.
- A box may be declared two ways. In CSS: an "inline-size" gives its width, a "max-lines" multiplied by the line height in force gives its height, and it is positioned so that its vertical centre is the y the field declares. Or by a "shape-inside" naming a rectangle of the template, as the verdicts graphic does. Where a field declares both, the rectangle is used.
- A "max-lines" is the field's line budget, and where declared it is the budget however the box was declared. A field declaring none takes the lines its rectangle admits, and a field declaring neither takes one line.
- A text longer than one line of its box shall be broken at word boundaries, up to the budget. Where it still does not fit, the size of the field shall be reduced by half a pixel at a time and the text broken again, until it does.
- **A text is never cut.** No field is truncated and no ellipsis is drawn, on any graphic. Whatever the league entered is drawn in full, however small the field must become to hold it. A single word wider than its box is broken within itself, and its pieces are not cut either.
- Below half the size the template declares, a field is no longer the size its designer intended, and a non-fatal error shall be reported naming it. That floor stops nothing: the reduction continues past it until the text fits. A truncated value is a wrong value drawn confidently — "Autodromo Enzo e Dino Ferra…" names no circuit — while a reduced one is the right value, merely smaller.
- The lines of a field are centred vertically within its box, so that a field taking one line and a field taking two sit alike, and a template reads as one design whether or not a value happened to wrap. A field taking a single line therefore sits exactly where the template drew it.
- Fields declaring a "shape-inside" are the exception and are laid from the top of their rectangle. Those carry prose — a steward's description, a steward's justification — and prose floating in the middle of a box it does not fill reads as a mistake rather than as a design.
- A template that declares a "max-lines" greater than one shall leave the vertical room a second line needs. The module measures a field against its box and against nothing else: it does not know what is drawn above or below, so a budget of two lines set on a field with room for one will overlap its neighbour instead of overrunning them sideways.
...
- One class of image carries one aspect throughout a template, and every slot of that class on that template shall carry it. A league authors one file per datum of a class, and a class serving slots of two aspects on one graphic would letterbox that one file wherever it did not match, which no authoring can answer.
- **The aspect a class carries is the league's to choose, and the module shall name no number.** The reference shall be read from the template being validated — the aspect the greater part of that class's slots declare — and the remaining slots reported against it. A template drawing every flag slot at 2:1 is valid; a template drawing most of them at 2:1 and one at 1:1 is not.
    - The classes bound by this are the track, team, flag, driver, weather and tyre classes.
    - The marker class is bound by none of it, drawing the square markers of a change of standing position and the stretching marks of a standings result cell and of an attendance total out of one directory, no one aspect serving all three. A marker slot at an aspect its fellows do not share shall be permitted, and shall be letterboxed.
    - The division logo class is bound by none of it either, and for a different reason: a league supplies one file per division rather than one per class, so two slots of the class upon one template may carry two aspects and each division may draw artwork for both. A division logo slot at an aspect its fellows do not share shall be permitted, and shall be letterboxed.
    - A class bound by none of it shall nonetheless not declare that it stretches, save the marker class. Freedom from one aspect is not licence to distort.
- **Agreement between templates shall not be required, and shall not be checked.** A class drawn by several templates may be drawn at a different aspect on each, and a league doing so shall have one file letterboxed wherever the aspects differ, unremarked. Requiring agreement would refuse the first template of any re-shaping, the others still disagreeing with it, and no league could move a class off the aspect it began with. The module shall document the consequence to leagues rather than refuse it.
- A slot may declare that it stretches, drawing an image to the room the field gives it rather than fitting the image within that room. Such a slot carries no aspect and the rules above do not bind it.
    - **A slot shall be permitted to declare this only where its class is the marker class**, and a slot of any other class declaring it is invalid and shall be refused whatever its aspect, the offending field being named. A driver portrait slot declaring that it stretches would draw every face of a league squashed to the shape of that slot, which nothing a league can author will correct; and were it merely exempted rather than refused, a template declaring it of every portrait slot would satisfy the agreement rule and be told nothing.
- A template declaring a slot at an aspect its class does not carry on that template is invalid and shall be refused, the offending field being named, and shall be reported as a fault of shape rather than as a missing field.
- The artwork the module itself supplies is drawn at one aspect per class, which a league that re-shapes a class cannot alter. Where such a file is drawn into a slot of another aspect it shall be stretched, and a notice shall be raised naming both aspects and saying that supplying the league's own file for that class answers it.
...
- A datum that is the vocabulary of the module itself, rather than a value a league supplies, shall ship as a file in the packaged directory of its class beside that class's fallback. A league did not choose such a vocabulary and cannot be incomplete against it, whether or not it has pointed the class at a directory of its own: where the configured directory holds neither the datum's own file nor a fallback of its own, the packaged directory shall be searched for the datum's own file before its "fallback.svg" is drawn. This is the one respect in which a file of the datum's own name sitting in the packaged directory is drawn for a league that did not put it there.
    - A datum is of this kind in either of two ways, which are one rule at two granularities and not two rules.
        ...
        - An individual reserved filename, in a class whose data are otherwise the league's own: "mystery", for a round concealed until it is run, and "other", for a driver who stated no nationality in particular. The datum settles it, the class being one whose other data are countries and circuits a league named.
    - The whole of a class such as the flag, the track image, the team badge or the driver portrait shall not be declared of this kind. A league supplying files for most of the countries it races in would otherwise be handed the module's file for the remainder, under a name it chose and for artwork it did not.
        - The bot may nonetheless obtain a league's own artwork upon the league's behalf, and for the driver portrait alone. Artwork so obtained is the league's own datum and not the vocabulary of the module: it is written into the directory the league configured and never into the packaged one, it is obtained only where the league has asked for it, and a file the league placed itself is never overwritten by it. No other class is obtained in this manner, and the rule above is otherwise undisturbed.
    - Where the file drawn is the datum's own, from whichever directory it came, the non-fatal error reported shall say that the file of the module was drawn and shall not state that a fallback was drawn. Nothing was substituted, and a league told otherwise would look for artwork it was never expected to supply.
...
- The normalized form of a datum, by which the file bearing it is named, is that datum trimmed of whitespace, stripped of diacritics, converted to lowercase, with every run of characters that is neither a letter nor a digit replaced by a single underscore, and any leading or trailing underscore removed. "Red Bull" becomes red_bull; "Force India (B)" becomes force_india_b. One rule serves every class of asset: a team name, a country, a track, a tyre compound and a condition of weather are all normalized by it.
    - It names a file and never a field of a template, every field of every collection being addressed by an ordinal. It is therefore bound by what a filename admits, and not by what the identifier of a node of an XML document admits.
    - The driver portrait is the one class this rule does not name, its files being keyed on the Discord user ID of the driver rather than on a normalized datum. The lineup specification below states the keying and the reason for it.
- The resolution of an asset has four outcomes and no others:
...
- Wherever else this document speaks of a directory holding, or not holding, a fallback, it means this two-tier check taken as a whole — the configured directory and the packaged directory of the class together — and not the configured directory alone.
...
- A fallback image is bound by the rules above as any other image is: plain SVG, authored at the aspect of the slot it fills, never padded at generation. One class carrying one aspect, its fallback is authored to that aspect as every other file of the class is; the fallback of a stretching class, and that of a class held to no aspect, is authored as its other files are.
- A class may be declared one whose fallback stands for the absence of artwork rather than for artwork that should have been supplied, whereupon the fallback is drawn and no error whatsoever is reported — neither that a fallback was drawn, nor that a packaged file was drawn off the shape of its slot. The division logo is such a class and is at present the only one. It is drawn only where a league's own template declares its field, having none of the module's own; a league that declares the field and supplies no artwork for a division is in the ordinary state of the class and not an incomplete one, and an error reported upon every graphic it posts would say nothing it could act upon.
    - The fallback shipped for such a class shall be an image with nothing drawn upon it, carrying a size and no mark, so that what is drawn in the absence of artwork is nothing.
    - The silence is declared for the class as a whole, where the silence of an absent datum below is declared field by field. The two are distinct: that one governs a datum never supplied, this one a datum supplied for which the league has drawn nothing.
...
- The flag of a driver is resolved from the country of the nationality recorded in their signup information, and not from the nationality itself. The module shall ship a map relating every canonical nationality adjective to the name of its country, so that "British" yields the country "United Kingdom" and thereby the file "united_kingdom.svg".
    ...
    - "Other", recorded for a driver who answered the question with "other", is not a country and gains none. It is carried through unchanged and resolves the file "other.svg", which is a reserved filename of the flag directory as "mystery.svg" is, and which the module shall ship. A driver holding no nationality at all is a distinct case and is drawn without a flag.
    - A driver created by test mode holds no signup information, and their flag is resolved from the nationality recorded upon the driver itself. Every place the module reads the nationality of a driver shall read it thus for such a driver, and from the signup information for every other.
...
- The map shall cover every UN member state. Three states share an English demonym with another country and would otherwise be unreachable behind it: Guinea-Bissau behind "Guinean", the Democratic Republic of the Congo behind "Congolese", and Dominica behind "Dominican". Each such state gains its own canonical adjective — "Bissau-Guinean", "Congolese (Kinshasa)" and "Dominican (Dominica)" respectively — so that it resolves its own flag rather than its more populous namesake's. The bare adjective is unaffected and continues to resolve the country it always has.
...
- The image of a team shall be searched for under a filename equal to the normalized form of that name, in every graphic that draws one: the lineup, the two results graphics, the two standings graphics, the attendance sheet and the verdict. No graphic addresses a team by its name in a field of the template, every field of every collection bearing an ordinal, so the name of a team reaches this module as a filename and in no other way.
- The names of teams are therefore constrained, so that the normalization above always yields a valid and unambiguous filename. The constraints belong to every graphic that draws a badge and not to the lineup alone.
    - These constraints shall hold whether or not the image module is enabled and whatever the toggles of "images config toggle" are set to. A name is constrained at the one moment it is set, and a league enabling the module later would otherwise hold names it could not correct without losing the history of the team.
    ...
    - A name beginning with a digit is admitted. The requirement that a name begin with a letter held only while the normalized form had to serve as the identifier of a node of an XML document, and is withdrawn with that use of it.
...
- A driver created by test mode shall be drawn by the mock name it was created with, in every type that draws drivers, and shall never be treated as an unoccupied seat. A division seated wholly with such drivers holds seated drivers, and no driver shall be fabricated over it.
- A seated driver carrying no nationality of their own shall be drawn without a flag, as a posting would draw them, and the reply shall name the count of drivers so drawn. A driver created by test mode carries the nationality the command creating it was given, and none where it was given none.
- A command of this family shall read the same data in the same way whether test mode is set or not. Test mode determines what data the server holds, never how a command of this family reads it.
- Every graphic a command of this family generates shall resolve its assets in the directories the league has configured, exactly as a posting for that division would resolve them, and shall answer a miss with the fallback of the class as the conventions above require, the packaged tier of that fallback included. A command of this family shall not substitute the packaged directories for those the league configured: the file named by the datum is sought in the configured directory alone, as it is for a posting. Withholding the packaged fallback tier from these commands would make a preview answer differently from the posting it exists to predict, and is not what this rule forbids.
...
### The vertical crop
- The calendar is cut as the conventions above require, at the crop point of the final round of the division, so that the height of the image is decided by the number of rounds the division holds and not by the height the template declares.
...
- The track map shall be searched for in the configured track image directory under a filename equal to the name of the track, normalized as the conventions above require, and resolved as the conventions above require, the number of the round standing for the field in any error reported.
- The flag of the round shall be searched for in the configured flag directory under a filename equal to the country recorded by the track object, normalized as the conventions above require, and resolved as they require, the number of the round standing for the field in any error reported.
...
- Once the calendar is to be posted and the "calendar" toggle of "images config toggle" is enabled, the image shall be generated following the rules above via modification of the SVG file, which shall then be converted to PNG and posted to the calendar channel configured for the division via "division calendar-channel" as an attachment of a message carrying no message text. The graphic draws the name of the division and states what it is, so a heading above it would repeat the graphic rather than introduce it.
- The textual calendar shall carry its heading, being the only thing that names the list of rounds it holds.
...
- The calendar graphic replaces the textual calendar in the calendar channel configured for the division, and in the "season review" command.
    - The image the review draws is output of a command and not the calendar of record. It shall neither be recorded as the calendar message of the division, nor cause the message in the calendar channel to be deleted, nor be posted to that channel.
...
- A lineup graphic represents the teams of one single division and the drivers occupying their seats. One graphic shall be generated per division; the same template file is reused for every division of the season, and holds nothing particular to any league.
- Its fields are addressed by the ordinal of the team, as the calendar's are addressed by the ordinal of the round. An ordinal stands for a place in the layout and never for a team in itself: which team is drawn in a given place is resolved from the division at the moment of generation, and the name and the badge of that team are the whole of what distinguishes one block from another.
- For generation of a lineup graphic, the template may have the following fields, among which the mandatory fields will be verified at template file setting and before generation:
    ...
    - For the reserve team of the division, which is a team of the division in its own right and never a subset of the seats of any other team:
        - reserve_group - Optional - Field acting as a container for every other field of the reserve team, which shall be removed in its entirety when the division fields no reserve drivers, or when the template declares no reserve slot at all
        ...
        - reserve_driver_<y>_name - Mandatory for <y> equal to 1 where the template declares any reserve slot, optional beyond it, and not demanded at all of a template declaring none - Field on which the name of the driver occupying seat number <y> of the reserve team is placed as text
...
- <x> is a value between 1 and the number of team blocks the template declares, numbered continuously from 1. The reserve team is never addressed via team_<x>_ fields, bearing a block of its own.
- <y> is a value between 1 and the number of seat slots the template declares within the block of ordinal <x>. The reserve team is configured with an unlimited number of seats, so the number of its slots is likewise decided solely by the template.
...
- The flag image of a driver shall be searched for in the configured flag directory under a filename equal to the country of the nationality recorded in their signup information, normalized and resolved as the conventions above require. Nationalities are recorded as adjectives in canonical form, and the module's map relates each to a country, so that "British" yields the country "United Kingdom" and thereby "united_kingdom.svg"; a driver who stated none has "Other" recorded, which is no country and is carried through to yield "other".
...
- The driver image of a driver shall be searched for in the configured driver image directory under a filename equal to the Discord user ID of that driver. It is keyed on the ID and not on a name because the ID is immutable where a name is not: both the username of an account and the display name that account carries on a server may be changed by the driver at any moment, and a portrait keyed on either would go missing on the day they changed it. Neither is therefore worth keying on.
    ...
    - Where the league has enabled it, the module shall obtain the portrait itself from the server profile picture of the driver's Discord account, writing it into the configured driver image directory under that same filename. A portrait so obtained is the league's own artwork obtained on the league's behalf, as the conventions of the fallback image provide.
        - The picture obtained shall be the one the account presents upon the server: the profile picture set for that server where the driver has set one, and the profile picture of the account itself otherwise.
        - A driver presenting neither, and carrying only the picture Discord generates for an account that has set none, shall have nothing obtained for them, and their portrait shall resolve to the fallback of the class exactly as an unsupplied portrait does. A generated picture stands for the absence of a portrait and not for a portrait.
        - A file the league placed in that directory itself shall never be overwritten, and no portrait shall be obtained for the driver whose ID names it. The module obtains a portrait only where the league has supplied none.
        - A portrait already obtained shall be obtained again only where the picture of the account has changed since it was last obtained.
        - A failure to obtain a portrait shall not prevent the graphic being drawn. The portrait resolves as it would have resolved had nothing been obtained, and the attempt is made again upon the next occasion.
- The team drawn in the block of ordinal <x> is the team standing at position <x> in the team list of the division being drawn, the reserve team excepted, that list being ordered as the division holds it. The correspondence is held by the division and is resolved afresh at each generation; it is recorded in no template.
    - The team list of a division shall be held in the order the teams were added to it, and in no order derived from their names. A team added takes the next free position, so that the teams already drawn do not move; a team renamed does not move at all.
    - The lineup posting path, the "images test" preview path and the team listing of "season review" shall read that one order, so that the ordinal a team occupies upon the graphic is the position it occupies in the text posted beside it.
    - The same ordinal may stand for a different team in another division, and for a differently named team in another league. Nothing of the block save the name and the badge varies with the team, so no template is thereby made wrong.
- The team image shall be searched for in the configured team image directory under a filename equal to the normalized team name, the reserve team included, and resolved as the conventions above require. It is resolved from the name of the team standing at that ordinal, and never from the ordinal itself.
...
- The number of reserve drivers of a division, in contrast, varies as drivers are assigned and unassigned over a season and cannot be known when the template is authored. Divergences in the reserve block are treated as follows:
    - reserve drivers in excess of the slots the template declares are a fatal error, naming them, where the template declares at least one slot;
    - a template declaring no reserve slot at all shall have its "reserve_group" field removed in its entirety, taking every other "reserve_" field with it, whatever number of reserve drivers the division fields, and no error nor warning shall be reported. This is not an overflow but an instruction: the league has authored a lineup on which reserves do not appear, and reporting it at every generation would report a decision already taken;
...
- The reserve slots a template declares are a ceiling upon the reserve drivers it can draw, and the reserve team is configured with an unlimited number of seats, so a division's reserves may grow past it as drivers are assigned. A command assigning a driver **to the reserve team** of a division, where that assignment would carry its reserve drivers past the slots the configured lineup template declares, shall be rejected and the assignment not applied rather than the overflow being reported at generation.
- A placement into any other team of the division joins no reserve block and shall never be refused upon this ground, however many reserves the division already carries. Where the template declares no reserve slot at all, nothing is measured and no placement is refused.
- No field of this graphic depends upon the team list of a league, so every field of it is verifiable against the template alone, at every moment the template is verified. There is no moment at which the template can be checked only against a stand-in, and no divergence of this graphic is ever reported as a warning.
    - When the template is configured, it shall be verified that the template declares "division_name", that it declares at least one team block and at least one seat slot within it, each numbered continuously from 1, and that the blocks declare "team_<x>_name" and "team_<x>_driver_<y>_name" throughout. Nothing of the reserve block is required: a template declaring none of it is valid. Where it declares reserve slots, they shall be numbered continuously from 1 and the first shall declare "reserve_driver_1_name", a block declared by halves being a fault of the template as a gap in any numbering is. A mandatory field that is absent is a fatal error rejecting the command.
    - At generation the counts the template declares are measured against the division being generated, an excess of teams or of seats on the side of the division being fatal as stated above.
    - At season review the same measurement is made against every division of the season, an excess being a failure of validation naming the division and the team or seat at fault. A season review is the last moment at which it can be corrected before the graphic is posted anywhere; a warning there would let a league approve a season every lineup of which then falls back to text.
    - These checks are made whether or not the "lineup" toggle of "images config toggle" is on. They report a template that cannot draw the season, and never restrict how a league may compose one.
    - They are made only where the image module is enabled, a league that has not enabled it having configured no template to measure.
...
- The lineup image shall replace the textual lineup in the following surfaces:
    ...
    - The "season review" command - the image replaces the textual lineup message, so that a league manager evaluates before approving the season the output their league will actually receive. The warning naming the teams of the division holding no role is a finding of the review and not of the lineup, and shall be posted whichever form the lineup takes.
...
- The tyre image of a qualifying entry shall be searched for in the configured tyre directory under a filename equal to the tyre compound recorded for the entry, normalized as the conventions above require, so that "Soft" yields soft. Where no tyre is recorded for the entry the fallback of the tyre directory shall be drawn upon the field and no error reported, a tyre being a value the submission of a session need not carry and the fallback standing for that absence rather than for a file that should have existed. Where the tyre directory holds no fallback the field shall be removed instead, and still no error reported. Where a tyre is recorded, its image is resolved as the conventions above require.
- The five compounds are a closed set the module itself defines and no league chooses, so the module shall ship a file for every one of them — "soft.svg", "medium.svg", "hard.svg", "intermediate.svg" and "wet.svg" — in the packaged tyre directory, beside the fallback of that directory, as it ships one for each type of weather. A league draws every qualifying graphic without authoring an icon, and a league pointing the directory elsewhere still draws the packaged directory's own matching icon for a compound missing from it, as the conventions above require of a closed-set class.
- The results of a session record the Discord role of the team an entry drove for, and not its name. The name to be placed, and the name to be normalized to search for the team image, shall be that of the team of the division holding that role, falling back to the name of the role itself should the division hold no such team. Normalization is that defined in the conventions above.
...
- For generation of a driver standings graphic, the template may have the following fields, among which the mandatory fields will be verified at template file setting and before generation:
    ...
    - The following further fields, by which the results obtained by the driver in each round of the division are displayed alongside the classification they produced. The whole of this catalogue is optional, a template declaring none of it drawing a classification alone:
        ...
        - For each row of ordinal <x> and each round of ordinal <z>:
            ...
            - row_<x>_round_<z>_sprint_qualifying_mark and row_<x>_round_<z>_feature_qualifying_mark - Optional - Fields on which the mark of the highlight the qualifying result of that round earns will be placed, drawn over the race cell of that session and searched for in the same directory
- For generation of a constructor standings graphic, the template may have the following fields, among which the mandatory fields will be verified at template file setting and before generation:
    ...
    - The following further fields, by which the results obtained in each round of the division by each driver who drove the team's cars are displayed alongside the classification they produced. The whole of this catalogue is optional, a template declaring none of it drawing a classification alone:
        ...
        - For each row of ordinal <x>, each round of ordinal <z> and each car of ordinal <w>:
            ...
            - row_<x>_round_<z>_driver_<w>_sprint_qualifying_mark and row_<x>_round_<z>_driver_<w>_feature_qualifying_mark - Optional - Fields on which the marks of the highlights that car's qualifying result earns will be placed, drawn as they are on the driver standings graphic and governed by the same rules
...
- A cell is drawn over three fields, each carrying an image independently of the others:
    - a background, carrying the mark of the highest highlight the race result earns;
...
- A qualifying result earns its layer by the same rules, and never the fastest lap layer, which is a matter of a race session alone.
- The qualifying layer of a session is drawn upon the cell of that session's **race** result, the raised qualifying figure sharing one run of text with the race result and having no position of its own.
...
- The slots of this class stretch to the field they fill and are not fitted within it, the class carrying no fixed aspect. A template may therefore draw a cell of one shape where another draws a cell of another. The class is the only one whose slots may declare that they stretch.
- Where a background is drawn, the colour of the result itself may be set with it, so that a value stays legible upon the background beneath it. That colour is named by the template, the background being an image that cannot supply it.
- **The colour of a result follows the background of its cell and nothing else.** Neither the fastest lap layer nor the qualifying layer shall set it. Each occupies a corner of the cell while the result is drawn inboard of them, over the background, so the background is the only thing the result is read against.
    - Where the template draws a qualifying result raised beside the race result, the raised figure stands upon the background of the race cell and its colour shall be set with that background for the same reason. That colour states nothing of the qualifying result.
...
- The number of rows a template declares is a ceiling upon the classification it can draw, and a division may field more teams, and more seats within them, than the template draws rows for, so a driver classification may grow past it as drivers are assigned. The drivers of the reserve team are no part of that classification, as stated above: seating a reserve grows it not at all, and is therefore never measured against this ceiling nor refused by it. It is verified against that ceiling at the moments the module is in a position to do so, and the input that would carry it past is rejected there rather than at generation:
...
- The constructors template is bounded the same way, by the number of teams a division fields, the reserve team not being one of them. No assignment of a driver adds a team, so that ceiling is verified at season review alone.
- The round columns of both templates are verified at season review against the longest calendar the season holds, a divergence being reported as the row ceilings are.
- Every such report shall name the drivers template and the constructors template separately and never as one pair. They are two files, and a manager told only that "the standings template" is too small cannot know which to enlarge.
...
- The results fabricated for a round shall classify the field in an order of that round's own, no two rounds of the grid classifying it alike, so that the grid drawn carries the variety a season produces rather than the same classification repeated across it. The round which the fastest lap falls to shall vary likewise, and shall fall to the winner of a round and to a driver outside the podium alike, so that every ground the fastest lap may be drawn upon can be evaluated.
    - The orders shall be derived from the round and the session and never drawn at random, a preview being required to draw the same picture on every invocation so that two renderings of one round may be compared against each other.
...
- The rectangle shall declare a width. It shall declare a height too, except where the field declares a "max-lines" that stands in for it. A wrapping field whose rectangle gives it no usable room is a fatal error: the field has been given nowhere to lay its text out, and drawing it as a single unwrapped line would put a steward's prose across the graphic with nothing reported.
...
- The number of lines the rectangle admits is its height divided by the line height in force, except where the field declares a "max-lines", which is then the budget instead. Where the text wrapped at the font size the template declares occupies more lines than the budget, the font size of the field shall be reduced and the text wrapped again until it fits. Passing below the floor of half the font size the template declares raises a non-fatal error naming the field and the verdict, and stops nothing: the reduction continues past the floor until the text fits. The text is never truncated and no ellipsis is drawn.
- The line height in force follows the font size. Where the size of a field is reduced, its line height is reduced in the same proportion, and where the budget is the rectangle's rather than a declared "max-lines", the number of lines it admits is worked out anew at the reduced line height. A field set smaller therefore holds more lines rather than the same number more widely spaced, and the reduction can win room where otherwise it could only narrow the lines it was already limited to.
...
- A field declaring an "inline-size" property and no "shape-inside" declares its box in CSS, as the conventions above define. It takes one line unless it declares a "max-lines", it is centred vertically on the y it declares rather than laid from a rectangle's top, and it is reduced to fit rather than truncated.
...
- The faults above - a "shape-inside" naming a rectangle the template does not hold, a field upon which no line height resolves where one is needed, a box of no usable width, and a "max-lines" that is not a positive whole number - are read from the template alone and need no data whatsoever. Each shall therefore be verified at every moment a template file is verified, when it is configured, at season review and before every generation alike, and shall be reported naming the field at fault and distinguishably from the other two. A league shall be told that its prose cannot be laid out at the moment it names the file, and not when a steward first writes a long verdict.
- The width of a text is measured against the font family, weight, style and size in force on the field — those it declares itself, and those it inherits — for which purpose the third dependency named at the head of this document is required. Where the font a field declares is not installed on the machine, the measurement shall be made against the font the converter would substitute for it and a non-fatal error reported naming the field and the font.
...
- The graphic relies upon no limit on the length of the free text of a verdict. It is for the league to declare a rectangle the longest verdicts its stewards write will fit at a size worth reading.
...
- The reduction of a field below half the size its template declares, and the substitution of a font a field declares, are non-fatal and reported as such.
...
- One banner is posted per **approval**, and not per function that posts a verdict beneath it. Approving a penalty review posts its penalty verdicts and then the attendance sanctions its scoring enforced, into the same channel and for the same round; one banner shall head the whole of that, and a second shall not be raised over the sanctions. Approving an appeals review posts a banner of its own, that being a later occasion and a separate run.
- **A banner shall be posted only where something follows it.** It shall be posted immediately before the first verdict graphic of its run, and not at all where the run produces none — no penalty applied, no correction applied and no sanction enforced shall post no banner. This holds of every occasion alike: a review approved with nothing staged, a run every record of which fails to resolve, and a sanction that is not announced because the division has no verdicts channel. An approval that posts no verdict shall read nothing from the database on the banner's account.
- **An attendance sanction is a verdict and shall be headed as one.** The sanctions of one round are enforced together and announced together, so one banner shall head them. Where they were enforced by a penalty approval that already raised one, they shall fall under it; where they were enforced by any other occasion — a recalculation following a pardon or an amendment — they shall raise one of their own. A sanction announced with nothing above it names no round to a reader (corrected 2026-09-09).
