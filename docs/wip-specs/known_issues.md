...
- The two reports the user raised were one defect: the misconfiguration notice and the all-fifteen check are the same code path seen from either end.
- **Fixed** on 2026-09-05, to one rule applied by every surface that judges a template: it blocks where the aspect drawing it is **on**, and is a warning where that aspect is **off**. `blocking_template_problems` filters `check_all_templates` through `ASPECT_TEMPLATES`, and `/season review` and `/season approve` both read it, so what the review shows as blocking is exactly what the approval refuses upon. The review still names a dormant fault, under its own heading, since a manager switching that aspect on later wants to have known. `/images config toggle` now **refuses** to switch an aspect on while its templates are at fault, leaving it off rather than storing an output that cannot draw; switching off is never refused. `/images config template-directory` is scoped the same way. Covered by `tests/unit/test_aspect_scoped_template_checks.py` and three cases in `tests/unit/test_image_cog_defers.py`.
...
**Fixed — the four season-setup commands did their work before answering, so the reply hit an expired interaction token.**
...
- **Fixed** on 2026-09-03 by deferring at the top of all four commands, ahead of any query, and sending every reply — errors included — through `followup`. Covered by `tests/unit/test_season_setup_defers.py`.
...
**Fixed — every mock driver seated during season setup was deleted by the next setup command.**
- Found on 2026-09-03, reported as a roster lost when the bot was terminated mid-setup.
- `save_pending_snapshot` (`src/services/season_service.py`) replaces a SETUP season by deleting it outright — divisions, teams, seats and rounds — and re-inserting the lot under new row IDs. It deleted every `driver_profiles` row carrying `is_test_driver = 1` seated in those divisions along the way, and `_snapshot_pending` (`src/cogs/season_cog.py`) calls it from **every** season-setup command: `/season setup`, `/division add`, `/round add` and `/round amend`.
- `add_test_driver` accepts a SETUP season as readily as an ACTIVE one — `_get_active_season_id` matches `status IN ('ACTIVE', 'SETUP')` — so a roster can be seated before approval, and then any further setup command destroyed it.
- The restart is what made it visible rather than what caused it: `recover_pending_setups` rebuilds the pending config faithfully, but the next setup command after the restart re-snapshots and takes the roster with it. The same loss happened without a restart, which is why it read as "termination lost the drivers".
...
**Fixed — `/test-mode roster list` failed outright once a division held more than about twenty drivers.**
- Found on 2026-09-04 on a division of twenty-three mock drivers.
- `roster_list` in `src/cogs/test_mode_cog.py` built the whole table into one string and sent it as a single message. Discord rejects a body over 2000 characters with `400 Invalid Form Body` rather than truncating it, so the manager received **no** listing at all and the failure appeared only in the log.
- The listing grows with the division, so the ceiling is reached by an ordinary test roster — twenty-three drivers is well within what a two-division league seats.
- **Fixed** on 2026-09-04 by `message_builder.paginate_fenced`, which splits a fenced listing on whole lines into messages that each fit, repeating the heading on every page and placing the footer on the last. The command defers and sends each page through `followup`. Splitting on whole lines is the point: a break inside a row or inside the ``` fence renders as garbage. Covered by `tests/unit/test_paginate_fenced.py` and a case in `tests/unit/test_test_mode_cog_roster.py`.

**Fixed — a mock driver restored by the season snapshot was invisible to `/season review` and to attendance.**
- Introduced on 2026-09-03 by the change that stops season setup deleting a seated mock roster, and reported from a running bot on 2026-09-04: a division showed 52 drivers in `/test-mode roster list` and an empty lineup in `/season review`, in the text output as well as the graphic. 51 of the 52 assignment rows carried a NULL `team_seat_id`; the one that drew was the only one that had not been through a rebuild.
- `restore_test_driver_seats` re-created each `driver_season_assignments` row without its `team_seat_id`, leaving the column NULL.
- Every reader of a seated driver reaches `team_seats` through that column — the lineup block of `/season review` at `src/cogs/season_cog.py:1466`, the same join at `:4291`, and nine sites across `src/services/attendance_service.py`. An inner join on a NULL discards the row, so each of them would have seen an empty division while the driver was seated and listed. Only a driver who had been through a snapshot restore was affected; one added normally carries the column and was always found.
- **Fixed** on 2026-09-04 by writing `team_seat_id` in the restore, naming the seat the driver was put back into. Pinned by `test_the_restored_assignment_carries_its_team_seat_id`, which asserts the join every reader makes.

**Fixed — a mock roster was seated in one season while `/season review` drew another, so the lineup came out empty.**
- Found on 2026-09-04: `/test-mode roster list` reported twenty-three seated drivers across a division's teams while the lineup graphic in `/season review` drew every team empty. No command reported a fault.
- `_get_active_season_id` in `src/services/test_roster_service.py` selected `WHERE server_id = ? AND status IN ('ACTIVE', 'SETUP')` and took the first row, with no `ORDER BY`. `get_setup_or_active_season` did the same with an explicit `LIMIT 1`. Which row either returned was uncontracted.
- On a server holding two live seasons the roster commands resolved one season's divisions and the review resolved the other's. `roster add` seated the drivers where its own lookup pointed, `roster list` read them back through the same lookup and so agreed with itself, and the review drew a different division that genuinely held nobody. Reproduced for an ACTIVE season beside a SETUP one and for two SETUP rows.
- The root cause was that the state existed at all. `/season setup` refused to start a second season, but nothing below that command enforced it, and both readers assumed a uniqueness the data did not have.
- **Fixed** on 2026-09-04 by making the state impossible: migration 049 adds a partial unique index over `seasons(server_id)` where the status is live, and repairs any database already holding two — keeping the ACTIVE season over a SETUP one, and cancelling rather than deleting the loser so nothing hanging off it is orphaned. Both readers are ordered ACTIVE-first to match `get_previewable_season`, as defence on a database predating the constraint. The image module spec had carried a rule for drawing whichever of two live seasons won — the wrong home for a core rule, and a rule for a state that can no longer arise — and it has been withdrawn. Covered by `tests/unit/test_one_live_season_per_server.py`.

**P3 — Every season-setup command rewrites the entire season, which is where its data losses and its timeouts both come from.**
- Found on 2026-09-04, after the two data-loss defects below were each traced to the same mechanism.
- `save_pending_snapshot` does not amend a SETUP season. It deletes the season row and every division, team, seat and round beneath it, then re-inserts the lot from the in-memory `PendingConfig` with new row ids. `_snapshot_pending` calls it from `/season setup`, `/division add`, `/round add` and `/round amend` alike, so adding one round rewrites everything the season holds.
- Measured on a two-division, eleven-team, twenty-four-round league built one `/round add` at a time: **2232 row writes to arrive at 116 stored rows**, roughly nineteen times the data actually kept. Each command also re-seeds every division's teams over two further connections apiece, each with its own commit.
- Two consequences already recorded separately are this one defect seen from different sides. The **data losses** below follow from it directly: anything not held in the `PendingConfig` — a channel, a points link, a seated driver — is destroyed unless it is explicitly saved and restored by name across the rebuild, and each such setting had to be added by hand. The **timeout** fixed on 2026-09-03 follows too: the rebuild is what outlasts Discord's three-second window, and it grows with the season, which is why the fault appeared on the second division and not the first. Deferring stopped the reply being lost; it did not make the command quick.
- It also sits behind the `## Database contention` P2 recorded below, which names this function first among the four long write transactions.
...
- The constraint is what saves it: nothing is corrupted and the transaction rolls back whole. But there is no `on_app_command_error` handler in `src/`, so the command replies nothing at all and only the log carries the traceback — the same silence `/test-mode roster remove` shows for the sibling defect recorded under Test mode.
- What a league sees: `/team remove` appears to hang on a team that has drivers in it, and works on an empty one. The remedy is to unplace the drivers first, which nothing tells them.
- Not fixed. The fix is to release the assignments — as the season snapshot now does — or to refuse the removal with a message naming the drivers still seated, which is the better answer: removing a team a league has placed drivers into is more likely a mistake than an intention.

**Fixed — every season-setup command unplaced every real driver already placed in the season being set up.**
- Found on 2026-09-04 while tracing the mock-driver case below, and verified against `save_pending_snapshot` directly.
- `save_pending_snapshot` (`src/services/season_service.py`) rebuilds a SETUP season by deleting it whole and re-inserting it. Among the deletions is `DELETE FROM driver_season_assignments WHERE division_id = ?`, which carries **no** `is_test_driver` filter, followed by `DELETE FROM team_seats` for every team instance of the division. Mock drivers are captured and restored by `restore_test_driver_seats`; real drivers are captured by nothing and restored by nothing.
- The driver's `driver_profiles` row survives, so the league is not obviously missing anybody. What is destroyed is their placement: the seat is gone and the season assignment with it, leaving `current_state = 'ASSIGNED'` on a profile that occupies no seat.
- It is reached by an ordinary sequence. `/driver assign` accepts a SETUP season — `get_setup_or_active_season` matches it, and only COMPLETED is refused — so a league that places its grid and then runs `/round add`, `/division add`, `/round amend` or a second `/season setup` loses every placement made so far. Measured on a one-driver division: profiles 1 → 1, assignments 1 → 0, occupied seats 1 → 0.
- What a league sees: nothing at all at the time. The command reports success, and the loss surfaces later as an empty lineup, a driver absent from a check-in, or a result submission rejecting a driver who is plainly in the league. Discord roles granted at placement are not revoked either, so the driver still looks placed.
- **Fixed** on 2026-09-04 by capturing every seated driver before the teardown, real and mock alike, and restoring them once the new teams exist. The capture no longer filters on `is_test_driver`, and `restore_test_driver_seats` is renamed `restore_driver_seats` to say so. A driver whose team or seat is genuinely gone — the division renamed, the team dropped from the setup — is left unseated for the manager to place again, and is never deleted. Covered by five real-driver cases in `tests/unit/test_season_snapshot_test_drivers.py`.
...
**Fixed — five tests constructed a Discord modal outside an event loop, and so failed on the Pi and nowhere else.**
...
- **Fixed** the same day. All five test bodies are now `async def`, which `pytest.ini`'s `asyncio_mode = auto` runs inside a loop; the change is to the signatures only, and the shared helpers stay synchronous because every other caller is already async. The rule now sits in the CLAUDE.md testing section so the drift is not repeated.
...
- **It did not reproduce on 2026-09-08**, once a full `/tmp` had been reclaimed: `pytest tests/ -q` ran all 4251 tests green in 292s, the 37 `rasteriser`-marked ones included and under exactly the full-suite load this entry describes. That is one run, and contention is load-dependent, so the entry stands rather than being closed. But it does mean a disk-full `/tmp` is the likelier first suspect for a full-run rasteriser failure here — see the entry below — and should be ruled out before this one is blamed.

**Fixed — retained pytest scratch filled the Pi's `/tmp`, and the suite then failed dishonestly across unrelated modules.**
- Found on 2026-09-08 while investigating why the suite was red here and green on CI.
- `/tmp` is a 923 MB tmpfs on this host. A full run leaves some 294 MB of `tmp_path` trees — 2408 directories, 281 MB of it the schema `.db` files the `tests/conftest.py` template mechanism copies per test — and pytest's default retains the last three sessions. Three runs therefore came to 888 MB and filled the disk outright.
- **The failures named the wrong culprit.** A full disk does not surface as "no space": Inkscape wrote 0-byte PNGs, so `test_shipped_assets.py` reported artwork that "rasterised to something suspiciously blank", and SQLite raised `database or disk is full` from `aiosqlite` inside fixtures, erroring tests in modules that had nothing to do with either. The state was reached silently, over three runs, and looked like a regression sweep.
- **Fixed** the same day, in two parts. `pytest.ini` sets `tmp_path_retention_count = 0`, which clears every earlier run's scratch at session start and — because `keep == 0` also suppresses the in-use lock file — its own at session end. `tests/conftest.py` gained a `pytest_sessionstart` sweep for the `f1-schema-` template directory, which sits outside the directories pytest owns and is otherwise only removed by an `atexit` a killed run never reaches.
- The cost is that a failing test's scratch is cleared too. `pytest tests/ -q -o tmp_path_retention_count=3` restores retention for one run when a failure needs inspecting. Both mechanisms are pinned by `tests/unit/test_scratch_retention.py`, since either is easy to tidy away without knowing what it prevents.
