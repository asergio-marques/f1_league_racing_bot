...
- **The member is told, seen by them alone**, that it did not finish. The reply names no
  exception: a Python type name tells a league manager nothing. It says the command may have
  been partly done, because nothing is rolled back and "nothing was changed" would be a
...
#: What the member who ran a failed command, button or form is told.
FAILURE_REPLY = (
    "❌ Something went wrong and this did not finish. It may have been partly done — "
    "check before running it again. The failure has been logged."
)
...
async def report_failure(
    ...
    try:
        if interaction.response.is_done():
            await interaction.followup.send(FAILURE_REPLY, ephemeral=True)
        else:
            await interaction.response.send_message(FAILURE_REPLY, ephemeral=True)
