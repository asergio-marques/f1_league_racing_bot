...
const CONTENTS = [
  'its tables and what each row means',
  'its services and what each owns',
  'what it registers with the scheduler and what a restart owes it',
  'what it posts, to which channel, and how it finds a message again',
  'how it fails',
  'the constraints a later reader might otherwise tune away',
]

const ASPECT_GROUPS = [
  { key: 'data', aspects: ['Tables'] },
  { key: 'behaviour', aspects: ['Services', 'Scheduling and restart'] },
  { key: 'effects', aspects: ['Posting', 'Failure', 'Constraints'] },
]

const CONCERNS = [
  {
    key: 'layering-and-data',
    brief: 'The module\'s layers and the direction of its dependencies: SQL outside the data layer, anything importing a cog, what its services import. Its data: tables it writes that another module also writes, tables it writes that it does not own, transactions held across a network call (#155), stored derived copies and what recomputes them (#238), rows leaking out of the data layer untyped.',
  },
  {
    key: 'composition-and-boundaries',
    brief: 'How the module is built and reached: its services\' constructors and what they take (bot, other services), how cogs reach them, whether its services match the container and base classes architecture.md settled. Its boundary: what it imports from other modules and what imports it (the survey\'s fan-in and module-dependency tables), its public surface, whether its files belong to it (classify() misfiles), and whether its largest files and flat services are the right shape.',
  },
  {
    key: 'runtime-output-failure',
    brief: 'Its time-driven work: jobs, their ids, what a restart re-registers and what a run missed while the bot was down owes (#426, #429). The event loop: blocking calls, tasks not kept, locks across awaits. Its output: every post, whether it goes the way architecture.md settled, whether its message id is kept and what finds it again (#189). Its failures: where each is caught, whether it reaches the one failure path, broad handlers, and what a league sees.',
  },
]
    ...
    findings: {
      ...
      items: {
        ...
        properties: {
          ...
          owner: { type: 'string', enum: ['#283', '#284', '#285', '#286', '#287', '#288', 'new tech-debt'] },
        },
      },
    },
    sound: {
...
const preamble = `Design pass for the ${module} module, issue ${issue}, at commit ${commit}. Read the issue first: gh issue view ${pass.issue} --json body,comments. Its "What the document should settle" names the topics ${pass.file} must cover beyond the six every module file covers. The module's rules are in ${pass.spec}; cite them, never restate them.${CORE_NOTE}`
...
Judge against ${GOVERNING} first, then the repository's recorded decisions, then .claude/skills/architecture-review/python-practices.md for what they leave open. This module's entries in the ratchet lists (tests/unit/test_architecture_rules.py and .importlinter) and its open issues are this pass's to correct: address each one. The owner of a finding is ${issue} when this pass can carry the correction, another pass's issue when the files are that module's, and "new tech-debt" when the correction is too large for this pass.
