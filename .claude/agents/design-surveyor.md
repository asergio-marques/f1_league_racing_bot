...
#282 names `driver_*` and `team_*` under signup although core owns the driver.
...
- **Tables.** Every table the scope writes or reads, from `src/db/migrations/`. For each: what
  ...
  read it; and any copy of a value derived from another table, naming what recomputes it.
...
- **Posting.** Everything the scope sends to Discord: what, to which channel, and through what
  (`OutputRouter`, a direct `.send`, an interaction response). For each post: whether its
  message id is kept, where, and what later finds the message again to edit or delete it.
