...
- [X] T001 Run `pytest tests/ -q` from the repo root and record the failure count and the failing
...
- [X] T002 [P] Confirm the rasteriser resolves by running
...
- [X] T003 Extend `RowSpec` in `src/models/image_catalogues.py`: widen `capacity` to `int | None`
...
- [X] T004 Add `declared_capacity(root)` and `capacity_for(root)` to `RowSpec` in
...
- [X] T005 Give `FieldCatalogue.all_mandatory_ids()`, `all_known_ids()` and `capacity()` an optional
      ...
      image type.
- [X] T006 Populate `CATALOGUES["calendar_template"]` in `src/models/image_catalogues.py` exactly as
...
- [X] T007 Update `declared_capacities()` in `src/models/image_catalogues.py` so a template-derived
...
- [X] T008 Update `_verify_against_data` in `src/services/image_render_service.py` to obtain the
      capacity via `catalogue.capacity(root)` rather than `catalogue.capacity()`, leaving the three
      checks and their order untouched.
- [X] T009 [P] Write `tests/unit/test_image_calendar_catalogue.py`: the catalogue's shape; capacity derived
...
- [X] T010 [P] [US1] Write `tests/test_image_calendar_fill.py`: a `CalendarDrawing` projects to the
...
- [X] T011 [P] [US1] Write `tests/test_image_calendar_crop.py`: for a template declaring M rounds, a
...
- [X] T012 [P] [US1] Author `resources/tracks/mystery.svg` — generic, not league-specific, at the same
...
- [X] T013 [US1] Create `src/services/image_calendar_service.py` with the `CalendarRound` and
      `CalendarDrawing` shapes from [data-model.md](./data-model.md).
- [X] T014 [US1] Implement `resolve_drawing(...)` in `src/services/image_calendar_service.py`: order
...
- [X] T015 [US1] Implement `build_fill_spec(drawing, root, config)` in
...
- [X] T016 [US1] Add the FR-026 notice to the crop step in `src/utils/svg_fill.py`: where the final
...
- [X] T017 [US1] Replace the generic sample for the calendar kind in
      ...
      sample unchanged.
- [X] T018 [US1] Wire the calendar kind through `/images test` in `src/cogs/image_cog.py`, and reject
...
- [X] T019 [P] [US2] Write `tests/test_calendar_validity.py`: a template missing a mandatory field is
...
- [X] T020 [US2] Extend `CatalogueLayer.check` in `src/services/image_validity_service.py` to derive
...
- [X] T021 [US2] Ensure the report from `evaluate_template` records the depth actually applied so the
...
- [X] T022 [US2] Extend the season review's images section in `src/cogs/season_cog.py` to report the
      ...
      approval.
- [X] T023 [US2] Ensure a faulty template refuses **approval** while leaving the review itself
      refusing nothing, per [contracts/commands.md](./contracts/commands.md).
- [X] T024 [US2] Add a round-capacity guard to the command that adds a round to a division: where the
...
- [X] T025 [P] [US3] Write `tests/test_calendar_post_service.py`: the message text is exactly
...
- [X] T026 [P] [US3] Add `src/db/migrations/040_calendar_message_id.sql` adding
      `calendar_message_id TEXT` to `divisions`, nullable, defaulting to `NULL`.
- [X] T027 [US3] Add `calendar_message_id: str | None = None` to `Division` in
...
- [X] T028 [US3] Create `src/services/calendar_post_service.py`: build a division's calendar in
      ...
      forms cannot drift.
- [X] T029 [US3] Replace the inline calendar block in `src/cogs/season_cog.py` (commented
...
- [X] T030 [US3] Report a fatal error to the log channel and to the approving manager, and fall back
      to the textual calendar for that division. Approval itself is not refused: the posting is not
      the thing commanded (XIV.7).
- [X] T031 [US3] Enqueue the **textual** calendar for retry where the posting fails for a Discord
...
- [X] T032 [P] [US4] Extend `tests/test_calendar_post_service.py`: a successful sync deletes the old
...
- [X] T033 [US4] Implement the replacement sequence in `src/services/calendar_post_service.py`:
...
- [X] T034 [US4] Add `/division calendar sync` per [contracts/commands.md](./contracts/commands.md):
...
- [X] T035 [US4] Confirm the deletion is **not** routed through the forecast flow's test-mode
...
- [X] T036 [P] Update the README's template-authoring section: `round_<x>_*` in the naming paragraph is
...
- [X] T037 [P] Update `resources/README.md` and the README's **Reserved filenames** paragraph now that
...
- **Baseline (recorded at T001, 2026-08-12)**: `22 failed, 1038 passed, 1 skipped` in 100s —
  20 in `tests/unit/test_attendance_tracking.py`, 1 in `tests/unit/test_rsvp_service.py`
  (`test_tier1_team_gets_reserve_first`), 1 in `tests/unit/test_season_end_service.py`
  (`test_check_schedules_when_all_phases_complete`). Matches the count CLAUDE.md documents.
  Rasteriser resolved at `C:\Program Files\Inkscape\bin\inkscape.exe`.
