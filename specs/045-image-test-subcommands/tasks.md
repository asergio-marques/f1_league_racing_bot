...
- [X] T040 Raise the constitution PATCH through `/speckit-constitution` for the two entity-inventory lines naming `images test verdicts` and the four `images test weather-*` **values**, which this feature makes subcommands. Last, and only if anything remains to raise. Never edit `.specify/memory/constitution.md` by hand
