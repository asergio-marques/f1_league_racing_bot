...
3. League managers apply penalties (or confirm no penalties) within the same channel before finalizing.
...
3. **Given** the penalty state is active, **When** any user who is not a league manager attempts to interact with the penalty interface, **Then** the bot ignores or rejects the interaction with a permissions error.
4. **Given** the penalty state is active, **When** a league manager attempts to submit new session result text in the submission channel, **Then** the bot rejects the input with a clear message that the round is in the penalty review state.
...
### User Story 2 — League manager enters, reviews, and adjusts staged penalties (Priority: P1)

Within the penalty state a league manager may stage one or more penalties against specific driver–session pairs. Time penalties (positive to add time, negative to subtract time) are accepted for race sessions; DSQ is accepted for any session type. The league manager can remove individual staged penalties before proceeding to the review step. When satisfied (or choosing no penalties) they advance to confirmation.
...
1. **Given** the penalty state is active and a race session is selected, **When** a league manager enters a driver ID and a time value in seconds (positive or negative integer), **Then** that signed time penalty is staged for that driver–session pair and the bot displays the updated staged list.
...
3. **Given** the penalty state is active, **When** a league manager applies a DSQ to a driver in any session type, **Then** a DSQ penalty entry is staged for that driver–session pair.
4. **Given** a league manager attempts to enter a time penalty for a driver in a qualifying session, **Then** the bot rejects the input and informs them that only DSQ penalties are accepted for qualifying sessions.
5. **Given** the staged list contains one or more entries, **When** a league manager removes a specific staged penalty by its list index or identifier, **Then** only that entry is removed; all other staged entries remain unchanged.
6. **Given** no penalties have been staged, **When** a league manager selects "No penalties", **Then** the wizard advances directly to the approval step presenting an empty penalty list.
7. **Given** one or more penalties are staged, **When** a league manager selects "No penalties", **Then** the bot requests explicit confirmation before clearing the staged list; if confirmed, the list is cleared and the wizard advances to the approval step with an empty list.
8. **Given** the review step is displayed, **When** the league manager selects "Make changes", **Then** the wizard returns to the penalty entry state and all previously staged penalties are restored exactly as they were.

---

### User Story 3 — League manager approves penalty review; round is finalized and final posts replace interim posts (Priority: P1)

A league manager that has reviewed the penalty list (empty or non-empty) presses approve. The bot applies penalties to the relevant session results, recomputes positions and points, and posts final results and standings to the respective channels — deleting the interim posts. The round state moves to FINALIZED and the submission channel closes.
...
1. **Given** test mode is active and a round is in the Post-Round Penalties state (not yet FINALIZED), **When** a league manager attempts to advance to the next scheduled event, **Then** the bot blocks the action and returns an error identifying the division and round number that must be finalized first.
2. **Given** test mode is active and the current round is in FINALIZED state, **When** a league manager advances to the next scheduled event, **Then** the advancement proceeds normally with no penalty-state block.
...
4. **Given** the bot is not running in test mode, **When** any round is in the Post-Round Penalties state and a league manager issues the advance command, **Then** the FINALIZED-state gate does not apply and the command is not blocked by this rule.
...
2. **Given** a round is already in FINALIZED state and new penalties must be applied retrospectively, **When** a league manager needs to correct results, **Then** they are directed to use the existing round amendment flow (User Story 6 of spec 019) as the appropriate correction path.
...
- A negative time penalty is valid and must be processed correctly. A negative time penalty that would reduce the driver's adjusted total race time below zero milliseconds is invalid; the bot MUST reject the staged entry at input time and prompt the admin to review their input before the penalty can be staged.
...
- Post-round penalty review may span several days or weeks while penalties are evaluated externally. A bot restart during this period is expected, not exceptional. The recovery guarantee is that season progress never stalls: the submission channel must remain open after a restart and the penalty prompt must be re-posted so the round can still be finalized. Staged-but-unapproved penalties are transient and are not preserved across a restart; the bot MUST indicate in the recovered prompt that a restart occurred and that any previously staged penalties must be re-entered.
...
- **FR-003**: League managers MUST be able to stage a signed integer time penalty (positive or negative, in seconds, non-zero) against a named driver in a race session. A staged time penalty MUST be rejected if applying it to the driver's recorded race time would produce a result below zero milliseconds; the bot MUST return an error prompting the league manager to review their input.
- **FR-004**: League managers MUST be able to stage a DSQ penalty against a named driver in any session type.
...
- **FR-007**: League managers MUST be able to remove individually staged penalties before approval.
...
- **FR-014**: If the bot restarts while a round is in the Post-Round Penalties state, the submission channel MUST remain open, the penalty prompt MUST be re-posted, and the round MUST remain finalizable after restart. Staged-but-unapproved penalties do not need to be persisted across a restart; the recovered prompt MUST include a visible notice that a restart occurred and any previously staged penalties must be re-entered.
...
- **NFR-001**: Every button and modal callback in the penalty wizard MUST respond within Discord's 3-second interaction window. Any operation that may exceed this threshold (e.g., `finalize_round`) MUST call `interaction.response.defer(ephemeral=True)` as its first statement and use `interaction.followup.send` for subsequent messages.

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: A league manager can complete the full round lifecycle — session submission through penalty review to finalization — without leaving the submission channel or invoking any commands outside the wizard.
