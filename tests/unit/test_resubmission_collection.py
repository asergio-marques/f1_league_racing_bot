"""Collecting a round's results again after a steward presses Resubmit.

Issue #208. `_resubmit_collection_task` was 99% untested. `test_result_resubmission.py` covers
the button: it deletes the round's results, reopens the channel and starts this task in the
background. That file stubs the task out, and nothing ran it — which is how the fault pinned
below went unnoticed.

**The task does not work today, and this file says so first.** It reads `season_id` and
`round_format` from `_get_round_context`, which selects neither. The first read raises
`KeyError`, inside a background task whose exception nobody retrieves. So by the time the
manager is told to paste the results again, those results are already deleted and nothing is
...
`_get_round_context` is patched to supply the two missing fields, so the loop's behaviour is
under test before anyone fixes the read — and whoever does fix it finds that behaviour already
pinned. The loop mirrors the first submission: sessions in racing order, a refused paste is
answered and the same session asked again, `CANCELLED` skips one, a fastest-lap override must
name a finisher, and the points configuration is chosen as before. It differs from the first
submission in two ways, and both are deliberate:

- **It reuses the existing channel.** The steward pressed Resubmit inside it, and it already
  holds the review's history.
...
def _context(fmt="NORMAL"):
    """What `_get_round_context` returns today, plus the two fields the task reads."""
    return {
        "server_id": SERVER_ID,
        "season_number": 4,
        "round_number": 3,
...
async def test_a_sprint_round_asks_for_all_four_sessions(tmp_path):
    ...
    await _run(_bot(db_path, [QUALI_PASTE, RACE_PASTE, QUALI_PASTE, RACE_PASTE]), fmt="SPRINT")
