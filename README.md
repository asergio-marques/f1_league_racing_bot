...
- **➕ Add Penalty** — prompts for the session, then opens a modal taking a driver mention or user ID, a penalty value (e.g. `+5s`, `-3s`, `DSQ`), a description and a justification. Both text fields are required and both are published in the verdict. Either one may mention a driver, but one that mentions a role, `@everyone` or `@here` is refused: a verdict notifies the drivers it names and never a whole group. So is one holding an emoji, your server's own or a standard one, or Discord markup (bold, italics, a heading or quote line, a masked link, or a channel, time or command mention), none of which a verdict drawn as a picture can show. A plain list or a bare link is fine. Positive and negative time penalties are supported for race sessions; only DSQ is accepted for qualifying sessions. Values are **whole seconds only**. Negative penalties are rejected if their magnitude exceeds the time penalties the driver already carries in that session, or if they would produce a negative total race time.
...
- **✅ Approve** — applies the staged penalties immediately (see below). It is greyed out while nothing is staged: to finish with nothing applied, use **No Penalties / Confirm**.
...
The separate **approval message**, carrying **✏️ Make Changes** to return to the penalty prompt with the staged list intact and **✅ Approve** to proceed, belongs to the **No Penalties / Confirm** path — it is what the bot posts when the round is being finalised with nothing applied, either because nothing was staged or because a staged list has just been cleared. It confirms the review as it stood when posted, so it is withdrawn as soon as anything is staged or removed, or **✏️ Make Changes** is pressed, and pressing **No Penalties / Confirm** again replaces it.
...
2. **The reports.** The bot lists the penalties the chosen sessions already carry, all together. Keep them, change them, remove them, or add new ones — and the same for the round's attendance pardons, each of which has its own **Remove Pardon** button here. Approving without touching anything leaves every decision exactly as it stood, and once approved the reports can no longer be changed: their buttons are refused. The first pass's **🔄 Resubmit Initial Results** button is not offered: to redo the classification, cancel and run the command again.
