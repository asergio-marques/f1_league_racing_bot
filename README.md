...
**Which version is running.** The bot writes it to its log as it starts, with the date and
time that version was made: `Running v0.5.0, made 2026-10-01T18:04:12+01:00` for a release,
or `Running v0.4.0-230, made …` for a copy taken between releases, the number counting the
changes merged since `v0.4.0`. Members see the same through **About** on the
[hub](#bot-hub-channel--set-the-hub)'s panel. Quote it when you report a problem. A copy
downloaded from GitHub, as a release's zip or with **Download ZIP**, carries both in the
`VERSION` file and needs nothing else; a copy made with `git clone` asks git for them
...
Both roles are the league's, not the signup module's. Disabling a module does not clear them. Neither is required unless the signup module is enabled; while it is, `/signup open` and confirming a season's configuration each refuse until both are set, both are still on the server, and the bot can grant the driver role.

**The driver role must be one the bot can grant.** `/bot driver-role` refuses `@everyone`, a role an integration manages (a bot's own role, Server Booster, a linked role), a role at or above the bot's own highest role, and any role at all while the bot lacks **Manage Roles** — each with the reason. Every approval grants it, and a grant Discord refuses would otherwise only reach the host's log.
...
> **Both roles are fixed once a season's configuration is confirmed — unless one is deleted.** From then until the season ends, both commands refuse. A role deleted from the server can be replaced all the same: nobody holds it any more, and without it no signup window could open. The bot gives the replacement to every driver — everyone Unassigned or Assigned, test drivers aside — and names anyone it could not give it to, for you to give by hand. That holds for the base role too, the one time the bot grants it: it knows its drivers but not the rest of your members, so the reply reminds you to give it to them yourself.
...
- where the signup module is enabled, its channel and the league's base role and driver role — both still on the server, and the driver role one the bot can grant — each fault naming the command that puts it right;
...
> **Three things hold the button back that the report does not otherwise show.** The season must hold at least one division that is not cancelled. Every signup must be settled: no driver may still be Unassigned, awaiting approval or correcting their signup — place them with `/driver assign`, turn them down with `/driver reject`, or finish reviewing their signup. And every division must have every channel it posts to — its **lineup** and **calendar** channels, and each channel an enabled module needs (weather; results, standings and verdicts; check-in and attendance) — set, and still on the server. The review lists every unsettled signup by name in its public report, names each channel at fault with its division and the command that sets it, and the approval refuses on the same reading. A channel the bot merely could not look up is not counted missing; only one Discord says is gone.
...
> **Mid-season, the review covers the new drivers only.** When a signup window closed mid-season leaves drivers to place, the season waits in *ongoing, placements* while you place them with `/driver assign`. `/season placements-review` then lists the drivers to confirm and each division's lineup as it will stand, and offers **✅ Confirm placements**, governed like the Approve button. Where the lineup is drawn as a picture, each lineup confirming will post is drawn with its new drivers in it and shown in place of its text.
>
> The button is held back, each reason told to you privately, while anything would stop the confirmation: a signup still unsettled; a channel a division posts to that is not set or has been deleted from the server; a fault in the image module's configuration — the rasteriser, a template an enabled output draws, the per-tier colours or the portrait settings; or a lineup confirming would post that will not draw. Pressing the button checks the signups, the channels and the image configuration again. The league's roles, the signup settings, the team list and the points configurations are not checked again mid-season: the season settled them when its configuration and its placements were first confirmed. Confirming grants the new drivers their division and team roles, posts each affected lineup once, and returns the season to ongoing. Until then the new drivers are outside the championship: no roles, no lineup, no check-in, no results or standings, while the season's rounds carry on without them.
>
...
> **The tyre column takes one of five compounds, and nothing else.** `Soft`, `Medium`, `Hard`, `Intermediate` and `Wet` are what a session can be run on, so a row naming anything else — `Ultrasoft`, `C3`, a typo — is sent back naming the five, rather than quietly recorded and drawn as a grey placeholder later.
>
> **Write them however you like.** Case, spacing and punctuation are ignored, and each compound answers to its plural and its single letter: `Soft`/`Softs`/`S`, `Medium`/`Mediums`/`M`, `Hard`/`Hards`/`H`, `Intermediate`/`Intermediates`/`Inter`/`Inters`/`I`, `Wet`/`Wets`/`ExWets`/`W`. Whichever you type, the canonical name is what gets recorded and what the results table and the graphic show.
>
> **Leaving it out is fine.** An empty column, or `N/A`, records no compound — a qualifying submission does not oblige one — and the graphic draws that as its own state rather than reporting a gap.
...
- **➕ Add Penalty** — prompts for the session, then opens a modal taking a driver mention or user ID, a penalty value (e.g. `+5s`, `-3s`, `DSQ`, `NFA`), a description and a justification. Both text fields are required and both are published in the verdict. Either one may mention a driver, but one that mentions a role, `@everyone` or `@here` is refused: a verdict notifies the drivers it names and never a whole group. So is one holding an emoji, your server's own or a standard one, or Discord markup (bold, italics, a heading or quote line, a masked link, or a channel, time or command mention), none of which a verdict drawn as a picture can show. A plain list or a bare link is fine. Positive and negative time penalties are supported for race sessions; only DSQ and NFA are accepted for qualifying sessions. Values are **whole seconds only**, and a penalty of zero seconds is refused. Negative penalties are rejected if their magnitude exceeds the time penalties the driver already carries in that session, or if they would produce a negative total race time.
  - **`NFA` — no further action — clears a driver.** Enter it, in any case, where the incident was investigated and no penalty follows. It changes nothing in the results, and its verdict names the driver and the incident and reads **None — no further action** where a sanction would stand. It is the way to announce a cleared driver: a zero-second penalty would be published as a sanction, which is why one is refused.
...
Approving the penalty stage posts a second prompt to the same channel, carrying **➕ Add Correction**, **No Changes / Confirm**, **✅ Approve** and a **Remove #N** per staged correction. A correction takes the same values as a penalty and is applied in the same way; it is the surface for overturning a sanction on appeal. An appeal turned down can be closed with `NFA`, which posts a verdict saying no further action follows.
...
1. **The classification.** Paste the corrected results in the channel — for several sessions, one after another as the bot asks for each, in running order. Nothing is written until the last paste is in, and a paste the bot refuses ends the whole amendment, the pastes already accepted included: have every session's classification ready before you start. Each paste, and any choice of points configuration you are asked for, must come within **five minutes**; past that the amendment ends the same way, and the bot tells you privately that it expired and can be run again. The format is exactly the same as a first submission; the two extra sanction columns the amendment used to take have been withdrawn, and a paste that still carries them is refused with a note saying so. The optional `FL: @Driver` fastest-lap override header (see above) is supported here as well.
...
| `session` | Choice | — | The one session to amend. If omitted the bot asks which to amend, and you may choose several |

> **Attendance is recalculated with it.** With the attendance module on, approving the amendment charges that round's attendance points again and carries the corrected totals through every later round whose penalties have been approved, then reposts the sheet and re-checks the auto-reserve and auto-sack thresholds. Each round's total is the driver's total **as at that round**, so the sheet posted and the thresholds checked are those of the division's **latest** approved round — where the totals now stand — and not those of the round you amended. Pardons already granted are kept unless you remove them in the report step. The amended round's attendance is **rebuilt from the corrected classification in both directions**: a driver added is marked present, and a driver taken out is marked absent and charged. Any auto-reserve or auto-sack the corrected totals warrant is applied **now**, against the latest round — never back-dated to the round where the limit would first have been crossed, and never undone where one already applied.

> **The whole division is reposted, in order.** A repost is a new message at the bottom of a channel, so replacing only the amended round would leave a five-round division reading 2, 3, 4, 5, 1. Every round goes up again in round order across the results, standings and verdicts channels, and the attendance sheet is reposted beside them. In each channel the new messages are posted **before** the old ones come down, so both are briefly visible and a failure part-way leaves you the board you already had rather than half of two. Posting is paced to stay inside Discord's rate limits, so a large division takes up to a minute.
...
>
> **The sheet shows one limit, because you can only have one.** Auto-reserve and auto-sack refuse each other, so the sheet carries a single plate that names whichever you set — `RESERVE AT 5`, `SACKED AT 8` — and the marks are measured against that same number. With neither set the plate leaves the picture entirely rather than standing there empty.
>
...
>
> **`verdict_banner` chooses whether the header above a batch of verdicts is drawn or written.** Every batch gets a header either way. With the toggle on, a review that applies penalties posts one banner to the division's verdicts channel before the first verdict, naming the season, the division and the round those decisions were taken on — and the grand prix and its flag where your track list holds a record for the round's circuit. A review that applies appeal corrections posts one of its own. With the toggle off, the same header is posted as plain text — `**Season 5 Premier Round 8**` — so you can always tell where one batch ends and the next begins.
>
> **A header only ever appears where something follows it.** A review you approve with nothing staged, and a round where nobody is penalised and nobody crosses an attendance threshold, post no header at all, drawn or written — you never get one standing over an empty channel.
>
...
>
> **A header that cannot be posted never delays a verdict.** Where the toggle is on and the render fails, the bot posts the written heading instead, so the batch is still identified. Where even that cannot be sent, every verdict follows as normal regardless — a header failing never costs you the decisions it heads.
>
> **Attendance sanctions are verdicts, and they get a header too.** The sackings and reserve moves the bot enforces itself are announced in the same channel, so they are headed like anything else. Where they were triggered by approving a penalty review, they fall under *that* approval's header rather than raising a second — you read one header over one run. Where they fire on their own, on a clean round with no penalties, or after a pardon or an amendment recalculates attendance, they raise one of their own.
>
> ⚠️ **A drawn banner carries no text, so Discord's search cannot find it.** This is the trade switching the toggle **on** makes: because the message body is empty, searching your verdicts channel for "Round 8" will not turn up that round's verdicts, and the only handle a search has is the attachment's filename, which the bot names `season5_division1_round8_verdict_banner.png`. Leave the toggle off and the header is written instead, which your league can search — you lose the picture, not the header.
...
>
> A driver with no tyre recorded draws the tyre directory's `fallback.svg` — an empty dashed ring with no letter in it — and says nothing about it: a tyre is a value a submission need not carry, so its absence is a state worth depicting rather than a gap worth reporting. Since every compound ships and a submission cannot record anything else, that file now stands for "none recorded" and nothing else.
>
> A driver who **did** record one draws that compound. The five compounds are the bot's own artwork and all five ship, so a qualifying graphic draws the right tyre on every row before your league has made any tyre artwork at all — see **What the bot named, the bot supplies** below.

> **Optional: shorten the picture to the rows it actually fills.** A standings, attendance, results or lineup template drawn for fifty rows gives a division of twenty thirty rows of empty canvas. The lineup counts **teams** rather than rows, so its points are `team_<x>_vertical_crop_point`, and its reserve block sits in `footer_group` with the caption so it rides up and stays beneath the last team. Give each row a `row_<x>_vertical_crop_point` — a zero-height shape whose **`y` is where the picture should end when that row is the last one drawn** — and the bot cuts the canvas there. Put the last row's crop point at the height your file declares, so a full-size division is still drawn whole; a file that does not is drawn to its crop point anyway and the log says so.
>
> **Optional: narrow the picture to the rounds the season actually runs.** The same cut, across the other axis. The attendance sheet and both standings sheets draw a column per round and are built for twelve; a division running eight left four columns' width standing empty — a quarter of a standings picture. Give each round a `round_<z>_horizontal_crop_point` and the bot cuts there instead. Unlike the row crop point, this one is a shape with **width**: its `x` is the right edge of that round's column, and its `x` plus its `width` is where the canvas should end. Anything drawn beyond that `x` — the attendance sheet's whole sanction column, its heading and its plate — is carried in by what the cut removes, so it keeps its place beside the last round. Put the last round's crop point so it *ends* at the width your file declares, so a full-length season is still drawn whole. Rules ruled *across* the sheet are shortened for you, exactly as rules ruled down it are.
>
...
> **The calendar needs a cancellation overlay on every round.** Give each round a `round_<x>_cancelled` element — a group or a shape drawn over that round, last in its group so it lies on top, and inside the round's own space, above its crop point. The bot leaves it on a cancelled round and removes it from every other, so draw it as you want a called-off round to look: the shipped calendar lays an almost opaque panel over the card with **CANCELLED** across it. It is **required**: a calendar drawing missing it for any round is refused, so a drawing of your own from before this change needs one added before the bot will accept it again. **Where you draw it is checked as well** — after the round's other fields, inside `round_<x>_group`, above that round's crop point, and big enough and near enough to cover the round. Copying one overlay into every round without moving it is refused by name.
...
>
> **The marks are artwork, not colours.** Each is an SVG in `resources/league/markers` — the same folder as the position-change markers and the attendance marks, all three being the bot's own vocabulary rather than values you chose — resolved exactly as team badges and flags are. Drop your own file in under one of these names and it is drawn; leave the folder empty and the bot's own marks are used.
>
> | File | Drawn for |
> |---|---|
> | `race_p1.svg` `race_p2.svg` `race_p3.svg` | A podium finish in the race — a full plate |
> | `race_points.svg` | Any other points-scoring race finish — a light tint |
> | `race_fastest_lap.svg` | The fastest lap — a triangle in the **top-left** corner |
...
> ```
>
> Save that as `race_points.svg` in your folder and no points finish is ever marked, while the podium plates and the fastest lap still are.
>
...
| `marker-directory` | `resources/league/markers` | Position-change markers, standings result marks, attendance limit marks |
...
| `division-logo-directory` | `resources/league/division-logos` | Division logos — optional, and drawn only where one of your own templates asks for one |
...
**You need not supply a `fallback.svg` of your own.** When a file for a specific value is not in your folder, the bot looks for a fallback there first and then in `resources/defaults/<class>/`. `markers/` is the one class with **two**: `position_change_fallback.svg` stands in for a missing movement marker and `standings_attendance_fallback.svg` for a missing standings or attendance mark, because a 64 × 64 arrow and a mark stretched into a result cell cannot share a placeholder. A plain `fallback.svg` there is still read, but only after the matching one — and since your folder's fallback is consulted *before* the bot's copy of the missing file, naming it for the shape it stands in for is what keeps it from being drawn in the wrong slot. Put eight of your ten team badges in `resources/league/teams` and every graphic still draws — the two without a badge get the packaged placeholder and a notice naming them. Put a `fallback.svg` in your own folder only when you want your placeholder rather than ours. The packaged directory is consulted for a *fallback* and nothing else: a file there under one of your teams' names is never drawn for you. What the bot itself named is the one exception — see below.
...
Your own file always wins where you supply one — this only ever fills a gap. Country flags, circuit maps, team badges, portraits and division logos are **not** covered by any of this: they are yours, and a missing one draws the placeholder. Replace the reserved artwork freely; keep the filenames. See [resources/README.md](resources/README.md) for the naming rule and the shape each class is drawn at.
...
**The standings are invented to show every case at once**, as far as the division's size allows. Their points are made up with the finishing order, so that both tables carry two entries level on points — second and third, separated as a countback separates them — and one on none. A round before them is invented too, so that one entry has gained places, one has lost them and the rest hold theirs. Where the division has a reserve seated, one is drawn in the drivers' table as a driver that earlier round did not hold, and for one round stands in for a driver who sat it out, while another team runs a car short and scores nothing. A large enough field carries a disqualification beside the retirement and the non-starter, in the results preview as on the standings grid.

**Sanctions a preview can draw.** A fabricated verdict carries five seconds added, ten seconds added, three seconds removed, a disqualification, or no further action. Those are the outcomes the bot can record and issue; a preview never draws one it cannot.
