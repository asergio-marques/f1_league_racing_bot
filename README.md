...
Opens a temporary, private **amend channel** (named `amend-S{N}-{slug}-R{N}`) in the same category as the bot commands channel, and **replays the round in three stages** — the way it was raced in the first place.

1. **The classification.** Paste the corrected results in the channel. The format is exactly the same as a first submission; the two extra sanction columns the amendment used to take have been withdrawn, and a paste that still carries them is refused with a note saying so. The optional `FL: @Driver` fastest-lap override header (see above) is supported here as well.
2. **The reports.** The bot lists the penalties the round already carries. Keep them, change them, remove them, or add new ones — and the same for the round's attendance pardons. Approving without touching anything leaves every decision exactly as it stood.
3. **The appeals.** The same again for the round's appeals. Approving this last stage is what commits the amendment.
...
> **Nothing is published until the last step.** The corrected classification is recorded when you paste it, but the round your drivers see is unchanged until you approve the appeals — it is never published half-amended. Amending one session also leaves the round's other sessions alone; their penalties and appeals are not reopened.

> **An amendment not carried through is undone.** Step one commits, so the bot does not leave a half-amended round standing: if the report and appeal steps go unapproved for half an hour — or the bot restarts mid-amendment — the round is put back exactly as it was, its results are reposted, and the log channel records an `AMEND_REVERTED` notice telling you it lapsed and can be run again.
...
> **Verdicts are re-announced, and the superseded ones removed.** The bot now records which message each verdict was posted in, so an amendment can replace it. All of a round's verdicts are announced again, in order, not only the ones that changed. A verdict announced *before* the bot began recording message ids cannot be found again — those are named in the log, and the old announcement is left standing for you to delete by hand rather than the bot pretending it replaced it.
